...
function PatientRow({ p, idx, onArchiveClick }) {
  ...
  // Check if trends data exists: from API field or calculated from scans
  const hasTrends = p.has_trends ?? (() => {
    const typeGroups = {}
    for (const s of scans) {
      if (!typeGroups[s.scan_type]) typeGroups[s.scan_type] = []
      typeGroups[s.scan_type].push(s)
    }
    return Object.values(typeGroups).some(group => group.length > 1)
  })()
      ...
      {scans.map((scan, i) => {
        ...
        return (
          <tr key={`${p.mrn}-${scan.scan_date}-${scan.scan_type}`} style={{ background: rowIdx % 2 === 0 ? '#fff' : '#f8fafc' }}>
            {isFirst ? (
              <>
                <td style={{ ...td, fontWeight: 600, rowSpan: scans.length }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    {p.name || '—'}
                    {hasTrends && (
                      <span style={{ display: 'inline-block', padding: '2px 6px', borderRadius: 4, fontSize: 10, fontWeight: 600, background: '#d1fae5', color: '#065f46' }}>
                        Trends
                      </span>
                    )}
                  </div>
...
export default function PatientTable() {
  ...
  if (!mountedRef.current && typeof window !== 'undefined' && typeof fetch === 'function') {
    mountedRef.current = true
    fetch('/api/list?page=0').then(r => r.json()).then(json => {
      setPatients(json.patients ?? [])
      setTotal(json.total ?? 0)
      setPages(json.pages ?? 1)
      setPage(json.page ?? 0)
