...
export async function selectScanAndHistory(mrn, targetDate = null) {
  ...
  if (targetDate) {
    // Find scan on specific date
    scan = dedupedScans.find(s => (s.scan_date ?? '').slice(0, 10) === targetDate)
    if (!scan) return null
    // Prior scans: all scans BEFORE target date
    priorScans = dedupedScans.filter(s => (s.scan_date ?? '').slice(0, 10) < targetDate)
  } else {
