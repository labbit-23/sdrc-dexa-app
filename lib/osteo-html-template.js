...
function pageForearm(data, P, lh, pageNum, total) {
  const { patient: pt, left_forearm: lfa, right_forearm: rfa, images } = data
  const hasLeft  = Object.keys(lfa).length > 0
  const hasRight = Object.keys(rfa).length > 0
  ...
  const rows = [
    ['Distal',  lfa.Distal,  rfa.Distal],
    ['Total',   lfa.Total,   rfa.Total ],
  ].map(([name, l, r]) => `
    <tr>
      <td ${st(`padding:8px;font-size:11px;color:${P.text};font-weight:600`)}>${name}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.text}`)}>${l?.bmd ? fmtBmd(l.bmd) : '—'}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.gray}`)}>${l?.T ? fmtScore(l.T) : '—'}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.gray}`)}>${l?.Z ? fmtScore(l.Z) : '—'}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.text}`)}>${r?.bmd ? fmtBmd(r.bmd) : '—'}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.gray}`)}>${r?.T ? fmtScore(r.T) : '—'}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.gray}`)}>${r?.Z ? fmtScore(r.Z) : '—'}</td>
    </tr>`).join('')
...
<div class="page">
  ...
  <div class="col" ${st('flex:1;gap:12px;min-height:0')}>
    <div class="card" ${st('flex-shrink:0')}>
      <div class="sec">Forearm Bone Mineral Density</div>
      <table ${st('width:100%;border-collapse:collapse;margin-top:12px')}>
        <thead><tr>
          ...
          <th colspan="3" ${st(`padding:8px;font-size:10px;font-weight:700;color:${P.gray};text-align:center;border-bottom:2px solid ${P.border}`)}><strong>Left Forearm</strong></th>
          <th colspan="3" ${st(`padding:8px;font-size:10px;font-weight:700;color:${P.gray};text-align:center;border-bottom:2px solid ${P.border}`)}><strong>Right Forearm</strong></th>
        </tr><tr>
      ...
      <div ${st(`font-size:9px;color:${P.gray};margin-top:12px;line-height:1.6`)}>
        <strong>BMD:</strong> g/cm² · <strong>T-score:</strong> Standard deviation from young adult mean · <strong>Z-score:</strong> Age-adjusted
      </div>
    </div>
  </div>
</div>`
