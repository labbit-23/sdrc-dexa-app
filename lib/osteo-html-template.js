...
function classColor(cls) {
  return cls === 'osteoporosis' ? C.red : cls === 'osteopenia' ? C.amber : cls === 'unknown' ? C.tealLt : C.greenLt
}
...
function classLabel(cls) {
  return cls === 'osteoporosis' ? 'Osteoporosis' : cls === 'osteopenia' ? 'Osteopenia' : cls === 'unknown' ? 'Z-Score Assessment' : 'Normal Bone Density'
}
...
function zScoreBadge(z) {
  const label = z <= -2.0 ? 'Well Below Expected for Age' : z <= -1.0 ? 'Below Expected for Age' : 'Within Expected for Age'
  const color = z <= -2.0 ? C.red : z <= -1.0 ? C.amber : C.greenLt
  return `<span class="tag" ${st(`color:${color};background:${color}22;border:1px solid ${color}44`)}>${label}</span>`
}
...
// noReferences — use neutral coloring (no reference data available)
function bmdRow(site, r, isTotal, P, suppressScores = false, hideYA = false, compact = false, noReferences = false) {
  const pd       = compact ? '4px 6px' : '8px 10px'
  const fs       = compact ? '10px' : '11px'
  const fsSmall  = compact ? '9px'  : '10px'
  // If no references, use neutral grey color; otherwise color by Z-score (young) or absolute BMD (adult)
  const bmdColor = noReferences ? P.gray : (r.T == null && r.Z != null ? (r.Z <= -2.0 ? C.red : r.Z <= -1.0 ? C.amber : C.greenLt) : (r.bmd > 0.9 ? C.greenLt : r.bmd > 0.7 ? C.amber : C.red))
  ...
  const scoreCells = suppressScores ? '' : (() => {
    ...
    const zColor = r.Z == null ? P.gray : r.Z <= -2.0 ? C.red : r.Z <= -1.0 ? C.amber : C.greenLt
    const tSign  = r.T != null && r.T >= 0 ? '+' : ''
    const zSign  = r.Z != null && r.Z >= 0 ? '+' : ''
    return `
    <td style="padding:${pd};font-size:${fs};font-family:monospace;text-align:right;color:${tColor};border-bottom:1px solid ${P.border}">${r.T != null ? tSign + r.T.toFixed(2) : '—'}</td>
    <td style="padding:${pd};font-size:${fs};font-family:monospace;text-align:right;color:${zColor};border-bottom:1px solid ${P.border}">${r.Z != null ? zSign + r.Z.toFixed(2) : '—'}</td>
...
function bmdTable(rows, P, suppressScores = false, hideYA = false, compact = false, noReferences = false) {
  ...
  <table style="width:100%;border-collapse:collapse">
    ...
    <tbody>
      ${rows.filter(([, r]) => r != null).map(([site, r, total]) => bmdRow(site, r, total, P, suppressScores, hideYA, compact, noReferences)).join('')}
    </tbody>
  </table>
...
function bmdRefChart(age, bmd, site, gender, P, dotLeft = null, dotRight = null) {
  ...
  const BMD_MIN = isHip ? 0.40 : 0.60  // hip: 0.40; spine/forearm: 0.60
  const BMD_MAX = isHip ? 1.20 : 1.45  // hip: 1.20; spine/forearm: 1.45
  ...
  //   Forearm  — gender-specific GE Lunar Radius 33% normative database
  const [peak, sd] = isHip
    ? [nRef.female.peak, nRef.female.sd]  // hip always female
    : isForearm
    ? (male ? [fRef.male.peak, fRef.male.sd] : [fRef.female.peak, fRef.female.sd])  // forearm: GE Lunar Radius ref
    : (male ? [sRef.male.peak, sRef.male.sd] : [sRef.female.peak, sRef.female.sd])  // spine: gender-specific
  ...
  const zoneRef = isHip
    ? nRef.female.curve  // hip always female
    : isForearm
    ? (male ? fRef.male.curve : fRef.female.curve)  // forearm: GE Lunar Radius ref
    : (male ? sRef.male.curve : sRef.female.curve)  // spine: gender-specific
  // Population mean line is gender-specific (for Z-score context)
  const meanRef = isHip
    ? (male ? nRef.male.curve : nRef.female.curve)  // hip: gender-specific for mean only
    : isForearm
    ? (male ? fRef.male.curve : fRef.female.curve)  // forearm: GE Lunar Radius ref
    : (male ? sRef.male.curve : sRef.female.curve)  // spine: gender-specific
  ...
  const clipId = `osteo-clip-${isHip ? 'hp' : 'sp'}`
  const totalW = W + 4
  // Title changes based on site type
  const title = isHip
    ? 'Femoral Neck · NHANES white female ref'
    : isForearm
    ? `Radius 33%${male ? ' · Male ref' : ' · Female ref'}`
    : `AP Spine L1–L4${male ? ' · Male ref' : ' · Female ref'}`

  return `
  <div ${st('flex-shrink:0')}>
    <div ${st(`font-size:6.5px;font-weight:600;color:${P.gray};margin-bottom:2px;text-transform:uppercase;letter-spacing:.4px`)}>${title}</div>
...
function page1(data, P, lh, pageNum, total) {
  ...
  // Show T-classification only if T-score available; Z-classification if only Z-score
  const showTClassification = spineT != null
  const showZClassification = !showTClassification && summary.spine_z != null
  ...
  const ageNote = summary.premenopausal
    ...
    : !showTClassification && !showZClassification
    ? `<div ${st(`font-size:9px;color:${C.amber};margin-top:8px;line-height:1.6;border-top:1px solid ${P.border};padding-top:8px`)}>
        ℹ Reference ranges are not available — measured values are shown.
      </div>` : ''
...
<div class="page">
  ...
  <div class="col" ${st('flex:1;gap:12px;min-height:0')}>
    ...
    <div class="card" ${st(`background:${showTClassification ? classBg(cls, P) : P.card};flex-shrink:0`)}>
      <div class="row" ${st('align-items:flex-end;gap:24px;margin-bottom:12px')}>
        <div ${st('flex:1;display:flex;flex-direction:column')}>
          ...
          <div ${st('margin-top:8px')}>${showTClassification ? classBadge(cls) : showZClassification ? zScoreBadge(summary.spine_z) : ''}</div>
          <div ${st('display:flex;align-items:baseline;gap:5px;margin-top:12px')}>
            <span ${st(`font-size:46px;font-weight:800;color:${showTClassification || showZClassification ? (showTClassification ? classColor(cls) : (summary.spine_z <= -2.0 ? C.red : summary.spine_z <= -1.0 ? C.amber : C.greenLt)) : P.gray};line-height:1`)}>${spineTotal?.bmd.toFixed(3) ?? '—'}</span>
        ...
        <div ${st('display:flex;flex-direction:column')}>
          ...
          <div ${st(`font-size:38px;font-weight:800;color:${showTClassification ? classColor(cls) : P.gray};line-height:1;margin-top:4px`)}>${spineT >= 0 ? '+' : ''}${spineT.toFixed(2)}</div>
        </div>
        <div ${st('display:flex;flex-direction:column')}>
          <div class="lbl">Z-Score</div>
          <div ${st(`font-size:9px;color:${P.gray};margin-top:2px`)}>vs same age &amp; sex peers</div>
          <div ${st('flex:1;min-height:8px')}></div>
          <div ${st(`font-size:38px;font-weight:800;color:${summary.spine_z != null && summary.spine_z <= -2 ? C.red : C.greenLt};line-height:1;margin-top:4px`)}>${summary.spine_z != null ? (summary.spine_z >= 0 ? '+' : '') + summary.spine_z.toFixed(2) : '—'}</div>
        </div>` : ''}
      </div>
...
/* ── Page 2: Dual Femur ──────────────────────────────────────────────────── */
function pageForearm(data, P, lh, pageNum, total) {
  const { patient: pt, forearm_summary: fa, images } = data
  if (!fa) return ''

  const fmt = (v) => v != null ? (v >= 0 ? '+' : '') + v.toFixed(2) : '—'
  const fmtBmd = (v) => v != null ? v.toFixed(3) : '—'
  const tScoreColor = (t) => {
    if (t == null) return P.gray
    if (t <= -2.5) return C.red
    if (t <= -1.0) return C.amber
    return C.greenLt
  }

  const buildTable = (side) => {
    const rows = fa.rows.map(({ region, isDiagnostic }) => {
      const data = side === 'left' ? fa.rows.find(r => r.region === region)?.left : fa.rows.find(r => r.region === region)?.right
      if (!data) return ''
      const tColor = tScoreColor(data?.T)
      const rowBg = region === 'Radius 33%' ? `background:${P.border}11;` : ''
      const fontWt = region === 'Radius 33%' ? 'font-weight:700;' : ''
      return `<tr style="${rowBg}">
        <td ${st(`padding:8px;font-size:11px;color:${P.text};${fontWt}`)}>${region}${isDiagnostic ? '' : ' ⁽ʳᵉᶠ⁾'}</td>
        <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.text}`)}>${fmtBmd(data?.bmd)}</td>
        <td ${st(`padding:8px;font-size:11px;text-align:right;color:${tColor};font-weight:${isDiagnostic ? '700' : '400'}`)}>${fmt(data?.T)}</td>
        <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.gray}`)}>${fmt(data?.Z)}</td>
      </tr>`
    }).filter(Boolean).join('')
    return rows
  }

  const leftTableRows = buildTable('left')
  const rightTableRows = buildTable('right')

  const primaryT = fa.primaryDiagnostic?.T
  const primaryClass = primaryT == null ? 'unknown' : (primaryT <= -2.5 ? 'osteoporosis' : primaryT <= -1.0 ? 'osteopenia' : 'normal')
  const primaryColor = tScoreColor(primaryT)

  const hasLeft = fa.rows.some(r => r.left)
  const hasRight = fa.rows.some(r => r.right)
  const hasImages = images && (images.left_forearm_overlay_url || images.right_forearm_overlay_url)
  const singleSide = hasLeft !== hasRight  // exactly one side scanned — the common case

  // Get Radius 33% BMD for chart
  const radius33Bmd = fa.primaryDiagnostic?.bmd

  const forearmTable = (side, rows) => `
    <div class="card" ${st('min-width:0')}>
      <div class="sec">${side} Forearm (Radius 33%)</div>
      <table ${st('width:100%;border-collapse:collapse;font-size:10px')}>
        <thead><tr>
          <th ${st(`padding:6px;text-align:left;border-bottom:1px solid ${P.border};color:${P.gray}`)}></th>
          <th ${st(`padding:6px;text-align:right;border-bottom:1px solid ${P.border};color:${P.gray}`)}>BMD</th>
          <th ${st(`padding:6px;text-align:right;border-bottom:1px solid ${P.border};color:${P.gray}`)}>T</th>
          <th ${st(`padding:6px;text-align:right;border-bottom:1px solid ${P.border};color:${P.gray}`)}>Z</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>`

  // Single-side layout — mirrors the spine page's hero image treatment (page1):
  // a full-size scan image spanning both rows, chart above the detail table.
  // Used whenever only one forearm was scanned, which is the normal case, so
  // that image isn't squeezed down to the width needed for a two-image layout.
  const singleSideBlock = (side) => {
    const label = side === 'left' ? 'Left Forearm' : 'Right Forearm'
    const overlayUrl = side === 'left' ? images?.left_forearm_overlay_url : images?.right_forearm_overlay_url
    const baseUrl = side === 'left' ? images?.left_forearm_url : images?.right_forearm_url
    const rows = side === 'left' ? leftTableRows : rightTableRows
    return `
    <div ${st('display:grid;grid-template-columns:auto 1fr;gap:14px;flex-shrink:0;align-items:start')}>
      <div ${st('grid-row:span 2;display:flex;flex-direction:column;gap:5px;align-items:center')}>
        <img src="${overlayUrl || baseUrl || ''}" alt="${label}"
             ${st('width:260px;max-width:260px;height:auto;border-radius:6px;display:block;object-fit:contain')}
             onerror="if(this.src!=='${baseUrl}'){this.src='${baseUrl}';this.onerror=null}else{this.style.display='none'}">
        <div ${st(`font-size:8.5px;color:${P.gray};letter-spacing:.3px;text-align:center`)}>${label}</div>
        <div ${st(`font-size:7.5px;color:${P.gray};opacity:.6;text-align:center`)}>Image not for diagnosis</div>
      </div>
      ${radius33Bmd != null ? densitometryChart(pt.age, radius33Bmd, 'forearm', pt.gender, P) : ''}
      ${forearmTable(label.split(' ')[0], rows)}
    </div>`
  }

  // Dual-side layout — kept compact (small image per side) since two images and
  // two tables need to share the page.
  const dualSideRow = (side) => {
    const label = side === 'left' ? 'Left Forearm' : 'Right Forearm'
    const overlayUrl = side === 'left' ? images?.left_forearm_overlay_url : images?.right_forearm_overlay_url
    const baseUrl = side === 'left' ? images?.left_forearm_url : images?.right_forearm_url
    const rows = side === 'left' ? leftTableRows : rightTableRows
    ...
    <div class="row" ${st('gap:14px;flex-shrink:0;align-items:flex-start')}>
      <div ${st('flex-shrink:0;display:flex;flex-direction:column;gap:4px;align-items:center;width:140px')}>
        <img src="${overlayUrl || baseUrl || ''}" alt="${label}"
             ...
             onerror="if(this.src!=='${baseUrl}'){this.src='${baseUrl}';this.onerror=null}else{this.style.display='none'}">
        <div ${st(`font-size:8.5px;color:${P.gray};text-align:center`)}>${label}</div>
        <div ${st(`font-size:7.5px;color:${P.gray};opacity:.6;text-align:center`)}>Image not for diagnosis</div>
      </div>
      ${radius33Bmd != null ? densitometryChart(pt.age, radius33Bmd, 'forearm', pt.gender, P) : ''}
      ${forearmTable(label.split(' ')[0], rows)}
  ...
  return `
<div class="page">
  ${header(pt, 'DEXA — Forearm', P, lh, pageNum, total)}
  <div class="col" ${st('flex:1;gap:12px;min-height:0')}>
    <div class="card" ${st(`background:${classBg(primaryClass, P)};flex-shrink:0`)}>
      <div class="row" ${st('align-items:flex-end;gap:24px;margin-bottom:10px')}>
        <div ${st('flex:1;display:flex;flex-direction:column')}>
          <div class="lbl">Forearm Classification (Radius 33%)</div>
          <div ${st('margin-top:8px')}>${classBadge(primaryClass)}</div>
          <div ${st(`font-size:11px;color:${P.gray};margin-top:8px;line-height:1.6`)}><strong>ISCD diagnostic site</strong></div>
        </div>
        ${primaryT != null ? `
        <div ${st('display:flex;flex-direction:column')}>
          <div class="lbl">T-Score</div>
          <div ${st(`font-size:9px;color:${P.gray};margin-top:2px`)}>Radius 33% (ISCD)</div>
          <div ${st('flex:1;min-height:8px')}></div>
          <div ${st(`font-size:38px;font-weight:800;color:${primaryColor};line-height:1;margin-top:4px`)}>${primaryT >= 0 ? '+' : ''}${primaryT.toFixed(2)}</div>
        </div>
        <div ${st('display:flex;flex-direction:column')}>
          <div class="lbl">Z-Score</div>
          <div ${st(`font-size:9px;color:${P.gray};margin-top:2px`)}>vs same age &amp; sex peers</div>
          <div ${st('flex:1;min-height:8px')}></div>
          <div ${st(`font-size:38px;font-weight:800;color:${P.gray};line-height:1;margin-top:4px`)}>${fa.primaryDiagnostic?.Z != null ? (fa.primaryDiagnostic.Z >= 0 ? '+' : '') + fa.primaryDiagnostic.Z.toFixed(2) : '—'}</div>
        </div>` : ''}
      </div>
      ${primaryT != null ? tScoreBar(primaryT, P) : ''}
    </div>

    <!-- ── Forearm: Image | Chart | Table ── -->
    ${hasLeft ? (singleSide ? singleSideBlock('left') : dualSideRow('left')) : ''}
    ${hasRight ? (singleSide ? singleSideBlock('right') : dualSideRow('right')) : ''}

    <div ${st(`font-size:8px;color:${P.gray};text-align:right`)}>ISCD: Radius 33% (one-third distal) is the standard diagnostic site for forearm BMD.</div>

    ${tScoreZScoreCard(P)}

    ${scanFootnotes([
      'Precision (LSC): ±0.033 g/cm² for Radius 33% (68% CI; repeat scans within this range may reflect measurement variability)',
      'Reference: GE Lunar Radius normative database (ages 20–40). T-scores provided for Radius sites only; Ulna and Both regions show reference values.',
    ], P)}
  </div>
</div>`
}
...
function page2(data, P, lh, pageNum, total) {
  ...
  // Only show classification if MDB provided valid reference ranges
  const hasHipReferences = (summary.lowest_hip_t != null || summary.lowest_hip_z != null)
  ...
  const lowestSideNote = summary.lowest_hip_t != null
    ? `ISCD diagnostic T-score: <strong ${st(`color:${classColor(hipClass ?? summary.overall_class)}`)}>${summary.lowest_hip_t >= 0 ? '+' : ''}${summary.lowest_hip_t.toFixed(2)}</strong>${hipSiteLabel ? ` — ${hipSiteLabel}` : ''}`
    : ''
  ...
  const hipSiteToDisplay = summary.lowest_hip_site ?? 'neck'
  const hipDotBmd = hipSiteToDisplay === 'total'
    ? lowestFemur?.Total?.bmd ?? lowestFemur?.Neck?.bmd ?? null
    : lowestFemur?.Neck?.bmd ?? lowestFemur?.Total?.bmd ?? null

  return `
<div class="page">
  ${header(pt, 'DEXA — Dual Femur', P, lh, pageNum, total)}
  <div class="col" ${st('flex:1;gap:12px;min-height:0')}>
    <div class="card" ${st(`background:${hasHipReferences ? classBg(hipClass ?? summary.overall_class, P) : P.card};flex-shrink:0`)}>
      <div class="row" ${st('align-items:flex-end;gap:24px;margin-bottom:10px')}>
        <div ${st('flex:1;display:flex;flex-direction:column')}>
          <div class="lbl">Hip Classification</div>
          <div ${st('margin-top:8px')}>${hasHipReferences ? classBadge(hipClass ?? summary.overall_class) : ''}</div>
          ${hasHipReferences ? `<div ${st(`font-size:11px;color:${P.gray};margin-top:8px;line-height:1.6`)}>${lowestSideNote}</div>` : `<div ${st(`font-size:11px;color:${P.gray};margin-top:8px;line-height:1.6`)}>ℹ Reference ranges are not available — measured values are shown.</div>`}
          ...
          <div ${st(`font-size:10px;color:${symmetry > 0.5 ? classColor(hipClass ?? summary.overall_class) : P.gray};margin-top:4px`)}>
            L/R Neck symmetry: Δ${symmetry.toFixed(2)} T-score
            ${symmetry > 0.5 ? '— asymmetry &gt;0.5 warrants clinical attention' : ''}
          </div>` : ''}
          <div ${st('flex:1;min-height:8px')}></div>
          ${hipDotBmd != null ? `
          <div ${st('display:flex;align-items:baseline;gap:4px')}>
            <span ${st(`font-size:36px;font-weight:800;color:${hasHipReferences ? classColor(hipClass ?? summary.overall_class) : P.gray};line-height:1`)}>${hipDotBmd.toFixed(3)}</span>
            <span ${st(`font-size:11px;color:${P.gray}`)}>g/cm²</span>
          </div>` : ''}
        </div>
        ...
        <div ${st('display:flex;flex-direction:column')}>
          ...
          <div ${st('flex:1;min-height:8px')}></div>
          <div ${st(`font-size:38px;font-weight:800;color:${hasHipReferences ? classColor(hipClass ?? summary.overall_class) : P.gray};line-height:1;margin-top:4px`)}>${summary.lowest_hip_t >= 0 ? '+' : ''}${summary.lowest_hip_t.toFixed(2)}</div>
        </div>
        <div ${st('display:flex;flex-direction:column')}>
          <div class="lbl">Z-Score</div>
          <div ${st(`font-size:9px;color:${P.gray};margin-top:2px`)}>vs same age &amp; sex peers</div>
          <div ${st('flex:1;min-height:8px')}></div>
          <div ${st(`font-size:38px;font-weight:800;color:${summary.lowest_hip_z != null && summary.lowest_hip_z <= -2 ? C.red : C.greenLt};line-height:1;margin-top:4px`)}>${summary.lowest_hip_z != null ? (summary.lowest_hip_z >= 0 ? '+' : '') + summary.lowest_hip_z.toFixed(2) : '—'}</div>
        </div>` : ''}
      </div>
    ...
    <div class="row" ${st('gap:14px;flex-shrink:0;align-items:flex-start')}>
      ...
      ${summary.left_implant || !hasHipReferences ? '' : densitometryChart(pt.age, hipSiteToDisplay === 'total' ? lf.Total?.bmd ?? lf.Neck?.bmd ?? null : lf.Neck?.bmd ?? lf.Total?.bmd ?? null, 'hip', pt.gender, P)}
      <div class="card" ${st(`flex:1${summary.left_implant ? `;background:${P.prosthesisBg};border-color:${C.blue}44` : ''}`)}>
        <div class="sec">Left Femur ${hipSiteToDisplay === 'total' ? '(Total)' : '(Neck)'}${summary.left_implant ? ` <span ${st(`display:inline-block;padding:2px 8px;border-radius:3px;background:${C.blue}33;color:${C.blueLt};font-size:9px;font-weight:700;letter-spacing:.5px;margin-left:8px`)}>PROSTHESIS</span>` : ''}</div>
        ${summary.left_implant
          ...
          : bmdTable(lfRows, P, false, true, true, !hasHipReferences)}
      </div>
    </div>` : ''}
    ...
    <div class="row" ${st('gap:14px;flex-shrink:0;align-items:flex-start')}>
      ...
      ${summary.right_implant || !hasHipReferences ? '' : densitometryChart(pt.age, hipSiteToDisplay === 'total' ? rf.Total?.bmd ?? rf.Neck?.bmd ?? null : rf.Neck?.bmd ?? rf.Total?.bmd ?? null, 'hip', pt.gender, P)}
      <div class="card" ${st(`flex:1${summary.right_implant ? `;background:${P.prosthesisBg};border-color:${C.blue}44` : ''}`)}>
        <div class="sec">Right Femur ${hipSiteToDisplay === 'total' ? '(Total)' : '(Neck)'}${summary.right_implant ? ` <span ${st(`display:inline-block;padding:2px 8px;border-radius:3px;background:${C.blue}33;color:${C.blueLt};font-size:9px;font-weight:700;letter-spacing:.5px;margin-left:8px`)}>PROSTHESIS</span>` : ''}</div>
        ${summary.right_implant
          ? `<div ${st(`font-size:11px;color:${C.blueLt};margin:6px 0 4px`)}>Excluded from analysis</div><div ${st(`font-size:9px;color:${P.gray};line-height:1.6`)}>Dedicated periprosthetic DXA may be considered if clinically required.</div>`
          : bmdTable(rfRows, P, false, true, true, !hasHipReferences)}
      </div>
    </div>` : ''}
...
function osteoSparkline(values, W, H, color) {
  ...
  const labels = values.map((v, i) => {
    if (v == null) return ''
    const x = (pad + (i / (values.length - 1)) * cW).toFixed(1)
    const y = (pad + (1 - (v - min) / range) * cH - 8).toFixed(1)
    return `<text x="${x}" y="${y}" text-anchor="middle" font-size="7" fill="${color}" font-weight="600">${v.toFixed(3)}</text>`
  }).join('')
...
function pageTrends(data, history, P, lh, pageNum, total) {
  ...
  /* ── A) BMD timeline with actual values ──────────────────────────────────── */
  const spineValsForTimeline = scans.map(sc => spineVals(sc))
  const timelineItems = scans.map((sc, i) => {
    const val = spineValsForTimeline[i]
    const cls = sc.summary?.overall_class ?? 'normal'
    const classCol = classColor(cls)
    const prev = i > 0 ? spineValsForTimeline[i - 1] : null
    const delta = prev && val.bmd ? val.bmd - prev.bmd : null
    const isCurrent = i === n - 1
    const arrowClass = delta != null ? (delta >= 0.028 ? 'greenLt' : delta <= -0.028 ? 'red' : 'gray') : 'gray'
    const arrow = delta != null ? (delta >= 0.028 ? 'up' : delta <= -0.028 ? 'down' : 'level') : ''
    return `
    <div ${st(`display:flex;flex-direction:column;align-items:center;gap:6px;flex:1;min-width:0`)}>
      <div ${st(`font-size:8px;color:${P.gray}`)}>${fmtDate(sc.patient?.scan_date)}</div>
      <div ${st(`display:flex;flex-direction:column;align-items:center;gap:2px`)}>
        <div ${st(`font-size:13px;font-weight:800;color:${classCol};font-family:monospace`)}>${val.bmd != null ? val.bmd.toFixed(3) : '-'}</div>
        <div ${st(`font-size:9px;color:${val.t != null ? (val.t <= -2.5 ? C.red : val.t <= -1 ? C.amber : C.greenLt) : P.gray}`)}>${val.t != null ? (val.t >= 0 ? '+' : '') + val.t.toFixed(2) : '-'}</div>
      </div>
      ${delta != null ? `<div ${st(`font-size:7px;color:${delta >= 0.028 ? C.greenLt : delta <= -0.028 ? C.red : P.gray}`)}>${delta >= 0 ? '+' : ''}${delta.toFixed(3)}</div>` : ''}
      ${isCurrent ? `<div ${st(`font-size:6.5px;color:${C.teal};font-weight:700`)}>Current</div>` : ''}
    </div>`
  }).join('')
  ...
  function siteCells(vals, prevVals, lsc, P, isCurrent) {
    ...
    return `<td style="padding:6px 8px;vertical-align:top;border-bottom:1px solid ${P.border}">
      <div ${st(`font-size:11px;font-weight:700;color:${P.text};font-family:monospace`)}>${bmd != null ? bmd.toFixed(3) : '—'}</div>
      <div ${st(`font-size:9px;color:${t != null ? (t <= -2.5 ? C.red : t <= -1 ? C.amber : C.greenLt) : P.gray}`)}>${t != null ? (t >= 0 ? '+' : '') + t.toFixed(2) : '—'}</div>
      ${delta}
    </td>`
  }
  ...
  const dateHeaderCells = scans.map((sc, i) => {
    const isCurrent = i === n - 1
    return `<th style="padding:6px 8px;font-size:9px;font-weight:${isCurrent ? 700 : 500};color:${isCurrent ? C.teal : P.gray};text-align:left;border-bottom:2px solid ${P.border}">${fmtDate(sc.patient?.scan_date)}${isCurrent ? ' ★' : ''}</th>`
  }).join('')
  ...
  <table style="width:100%;border-collapse:collapse;font-size:10px">
    ...
    <tbody>
      ...
      <tr>
        <td style="padding:6px 8px;font-size:11px;font-weight:600;color:${P.grayLt};border-bottom:1px solid ${P.border};white-space:nowrap">Hip (diagnostic)</td>
        ${hipBodyCells}
      </tr>
    </tbody>
  </table>
  ...
  <div class="card" ${st('flex-shrink:0;margin-top:10px')}>
    <div class="sec">Body Fat % Trends (Estimated)</div>
    <div ${st(`font-size:8px;color:${P.gray};margin-bottom:8px;font-style:italic`)}>Estimated from Hips and Spine DEXA — not a full body composition scan</div>
    <table style="width:100%;border-collapse:collapse;font-size:10px">
      <thead>
        <tr>
...
function page3(data, P, lh, pageNum, total) {
  ...
  const lowestTVal = summary.overall_t
  const lowestZVal = summary.overall_z
  ...
  // Determine if we show WHO classification or Z-score badge
  const hasValidTScores = summary.lowest_hip_t != null || summary.spine_t != null || summary.forearm_t != null
  const hasValidZScores = summary.lowest_hip_z != null || summary.spine_z != null || summary.forearm_z != null
  const showTClassificationBadge = hasValidTScores
  const showZClassificationBadge = !showTClassificationBadge && hasValidZScores

  // Only show classification if MDB provided valid reference ranges
  const hasValidReferences = (summary.lowest_hip_t != null || summary.lowest_hip_z != null ||
                              summary.spine_t != null || summary.spine_z != null ||
                              summary.forearm_t != null || summary.forearm_z != null)

  const recommendations = []

  if (!hasValidReferences) {
    // No reference ranges available — don't classify, stay silent
  } else if (summary.premenopausal || lowestTVal == null) {
    // Premenopausal or young patients (Z-score only): show Z-score assessment, not WHO classification
    const zScoreVal = summary.spine_z ?? summary.lowest_hip_z
    const isBelowExpected = zScoreVal != null && zScoreVal <= -1.0
    const isSeverelyLow = zScoreVal != null && zScoreVal <= -2.0
    recommendations.push({
      status: isBelowExpected ? (isSeverelyLow ? 'alert' : 'warn') : 'good',
      title:  `Bone Status: ${isSeverelyLow ? 'Well Below Expected Range for Age' : isBelowExpected ? 'Below Expected Range for Age' : 'Normal for Age'}`,
      body:   `Z-Score ${zScoreVal?.toFixed(2) ?? '—'} — Age-matched comparison shows bone density ${isSeverelyLow ? 'well below' : isBelowExpected ? 'below' : 'within'} expected range for age-matched peers. ${isBelowExpected ? (isSeverelyLow ? 'Urgent secondary cause investigation recommended (malabsorption, vitamin D deficiency, hypogonadism, steroid use, underlying endocrine or metabolic disorder).' : 'Secondary cause investigation warranted (vitamin D deficiency, calcium malabsorption, hypogonadism, steroid exposure).') : 'Consistent with healthy age-related bone status.'}`,
    })
  } else {
    const tDisplay = lowestTVal != null ? (lowestTVal >= 0 ? '+' : '') + lowestTVal.toFixed(2) : '—'
    let body = ''
    if (cls === 'osteoporosis') {
    ...
    if (lowestZVal != null) {
      const zDisplay = (lowestZVal >= 0 ? '+' : '') + lowestZVal.toFixed(2)
      if (lowestZVal <= -2.0) {
...
<div class="page">
  ...
  <div ${st(`flex-shrink:0;display:flex;align-items:center;gap:16px;padding:12px 16px;border-radius:8px;margin-bottom:14px;background:${clsColor}18;border:1px solid ${clsColor}44`)}>
    ...
    <div ${st(`font-size:11px;color:${P.gray};line-height:1.7`)}>
      Lowest T-score: <strong ${st(`color:${clsColor}`)}>${lowestTVal != null ? (lowestTVal >= 0 ? '+' : '') + lowestTVal.toFixed(2) : '—'}</strong>
      ${summary.spine_t != null ? `&nbsp;·&nbsp; Spine L1–L4: ${(summary.spine_t >= 0 ? '+' : '') + summary.spine_t.toFixed(2)}` : ''}
      ${summary.lowest_hip_t != null ? `&nbsp;·&nbsp; Hip: ${(summary.lowest_hip_t >= 0 ? '+' : '') + summary.lowest_hip_t.toFixed(2)} (${summary.lowest_hip_side})` : ''}
      ${summary.forearm_t != null ? `&nbsp;·&nbsp; ${summary.forearm_side === 'left' ? 'Left' : 'Right'} Forearm (Radius 33%): ${(summary.forearm_t >= 0 ? '+' : '') + summary.forearm_t.toFixed(2)}` : ''}
    </div>
  </div>` : showZClassificationBadge ? `
  <div ${st(`flex-shrink:0;display:flex;align-items:center;gap:16px;padding:12px 16px;border-radius:8px;margin-bottom:14px;background:${P.card};border:1px solid ${P.border}`)}>
    <div ${st('flex-shrink:0;display:flex;flex-direction:column;gap:3px')}>
      ${zScoreBadge(lowestZVal)}
    </div>
    <div ${st(`font-size:11px;color:${P.gray};line-height:1.7`)}>
      Z-score: <strong>${lowestZVal != null ? (lowestZVal >= 0 ? '+' : '') + lowestZVal.toFixed(2) : '—'}</strong>
      ${summary.spine_z != null ? `&nbsp;·&nbsp; Spine L1–L4: ${(summary.spine_z >= 0 ? '+' : '') + summary.spine_z.toFixed(2)}` : ''}
      ${summary.lowest_hip_z != null ? `&nbsp;·&nbsp; Hip: ${(summary.lowest_hip_z >= 0 ? '+' : '') + summary.lowest_hip_z.toFixed(2)}` : ''}
    </div>
  </div>` : `
  <div ${st(`flex-shrink:0;display:flex;align-items:center;gap:16px;padding:12px 16px;border-radius:8px;margin-bottom:14px;background:${P.card};border:1px solid ${P.border}`)}>
    <div style="flex:1">
      <div ${st(`font-size:13px;font-weight:700;color:${P.text};margin-bottom:5px`)}}>Reference Ranges Not Available</div>
      <div ${st(`font-size:11px;color:${P.gray};line-height:1.6`)}}>ℹ Reference ranges are not available for this patient. Measured values are shown. Clinical interpretation cannot be provided without comparison standards.</div>
    </div>
  </div>`}
...
export function generateOsteoHtml(data, { dark = false, letterhead = false, history = [], preview = false } = {}) {
  ...
  const hasLeftForearm = Object.keys(data.left_forearm ?? {}).length > 0
  const hasRightForearm = Object.keys(data.right_forearm ?? {}).length > 0
