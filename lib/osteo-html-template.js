  ...
  <div class="col" ${st('flex:1;gap:14px;min-height:0')}>
    ...
    <div class="row" ${st('gap:14px;flex:1;min-height:0;align-items:flex-start')}>
      <div ${st('flex-shrink:0;display:flex;flex-direction:column;gap:6px;align-items:center')}>
        ...
        <div ${st(`font-size:9px;color:${P.gray};letter-spacing:.4px`)}>AP Lumbar Spine</div>
      </div>
...
<div class="page">
  ...
  <div class="col" ${st('flex:1;gap:14px;min-height:0')}>
    ...
    <div class="row" ${st('gap:14px;flex-shrink:0')}>
      ${hasLeft ? `
      <div ${st('flex:1;display:flex;align-items:center;justify-content:center;overflow:hidden;border-radius:8px;max-height:200px')}>
...
function compCard(comp, pt, P) {
  ...
  const items = [
    { label: 'Total Fat',    val: comp.fat_kg   != null ? `${comp.fat_kg} kg` : '—' },
    { label: 'Lean Mass',   val: comp.lean_kg  != null ? `${comp.lean_kg} kg` : '—' },
    { label: 'Body Fat %',  val: pct(comp.fat_pct) },
    { label: 'FMI',         val: fmt1(comp.fmi), unit: 'kg/m²' },
    { label: 'LMI',         val: fmt1(comp.lmi), unit: 'kg/m²' },
    { label: 'Android Fat', val: pct(comp.android_fat_pct) },
    { label: 'Gynoid Fat',  val: pct(comp.gynoid_fat_pct) },
    { label: 'A/G Ratio',   val: ag_ratio ?? '—' },
  ]
  return `
  <div ${st(`background:${P.card};border:1px solid ${P.border};border-radius:8px;padding:12px 16px;margin-bottom:12px`)}>
    <div ${st(`font-size:12px;font-weight:700;color:${P.text};margin-bottom:8px`)}>
      Estimated Body Composition
      <span ${st(`font-size:9px;font-weight:400;color:${P.gray};margin-left:6px`)}>GE Lunar AP-spine analysis (estimated, not full DXA body composition)</span>
    </div>
    <div ${st('display:grid;grid-template-columns:repeat(4,1fr);gap:6px 12px')}>
      ${items.map(it => `
        <div>
