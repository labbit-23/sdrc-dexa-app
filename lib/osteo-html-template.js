    ...
    <div class="card" ${st(`background:${showTClassification ? classBg(cls, P) : P.card};flex-shrink:0`)}>
      <div class="row" ${st('align-items:flex-end;gap:24px;margin-bottom:12px')}>
        ...
        <div ${st('display:flex;flex-direction:column')}>
          ...
          <div ${st(`font-size:38px;font-weight:800;color:${showTClassification ? classColor(cls) : P.gray};line-height:1;margin-top:4px`)}>${spineT >= 0 ? '+' : ''}${spineT.toFixed(1)}</div>
        </div>
        <div ${st('display:flex;flex-direction:column')}>
    ...
    <div class="card" ${st(`background:${classBg(primaryClass, P)};flex-shrink:0`)}>
      <div class="row" ${st('align-items:flex-end;gap:24px;margin-bottom:10px')}>
        ...
        <div ${st('display:flex;flex-direction:column')}>
          ...
          <div ${st('flex:1;min-height:8px')}></div>
          <div ${st(`font-size:38px;font-weight:800;color:${primaryColor};line-height:1;margin-top:4px`)}>${primaryT >= 0 ? '+' : ''}${primaryT.toFixed(1)}</div>
        </div>
        <div ${st('display:flex;flex-direction:column')}>
          <div class="lbl">Z-Score</div>
          <div ${st(`font-size:9px;color:${P.gray};margin-top:2px`)}>vs same age &amp; sex peers</div>
          <div ${st('flex:1;min-height:8px')}></div>
          <div ${st(`font-size:38px;font-weight:800;color:${P.gray};line-height:1;margin-top:4px`)}>${fa.primaryDiagnostic?.Z != null ? (fa.primaryDiagnostic.Z >= 0 ? '+' : '') + fa.primaryDiagnostic.Z.toFixed(1) : '—'}</div>
        </div>` : ''}
      </div>
    ...
    <div class="card" ${st(`background:${hasHipReferences ? classBg(hipClass ?? summary.overall_class, P) : P.card};flex-shrink:0`)}>
      <div class="row" ${st('align-items:flex-end;gap:24px;margin-bottom:10px')}>
        ...
        <div ${st('display:flex;flex-direction:column')}>
          ...
          <div ${st('flex:1;min-height:8px')}></div>
          <div ${st(`font-size:38px;font-weight:800;color:${hasHipReferences ? classColor(hipClass ?? summary.overall_class) : P.gray};line-height:1;margin-top:4px`)}>${summary.lowest_hip_t >= 0 ? '+' : ''}${summary.lowest_hip_t.toFixed(1)}</div>
        </div>
        <div ${st('display:flex;flex-direction:column')}>
          <div class="lbl">Z-Score</div>
          <div ${st(`font-size:9px;color:${P.gray};margin-top:2px`)}>vs same age &amp; sex peers</div>
  ...
  const timelineItems = scans.map((sc, i) => {
    ...
    <div ${st(`display:flex;flex-direction:column;align-items:center;gap:6px;flex:1;min-width:0`)}>
      ...
      <div ${st(`display:flex;flex-direction:column;align-items:center;gap:2px`)}>
        ...
        <div ${st(`font-size:9px;color:${val.t != null ? (val.t <= -2.5 ? C.red : val.t <= -1 ? C.amber : C.greenLt) : P.gray}`)}>${val.t != null ? (val.t >= 0 ? '+' : '') + val.t.toFixed(1) : '-'}</div>
      </div>
...
<div class="page">
  ...
  <div ${st(`flex-shrink:0;display:flex;align-items:center;gap:16px;padding:12px 16px;border-radius:8px;margin-bottom:14px;background:${clsColor}18;border:1px solid ${clsColor}44`)}>
    ...
    <div ${st(`font-size:11px;color:${P.gray};line-height:1.7`)}>
      ...
      ${summary.spine_t != null ? `&nbsp;·&nbsp; Spine L1–L4: ${(summary.spine_t >= 0 ? '+' : '') + summary.spine_t.toFixed(1)}` : ''}
      ${summary.lowest_hip_t != null ? `&nbsp;·&nbsp; Hip: ${(summary.lowest_hip_t >= 0 ? '+' : '') + summary.lowest_hip_t.toFixed(1)} (${summary.lowest_hip_side})` : ''}
      ${summary.forearm_t != null && summary.spine_t == null && summary.lowest_hip_t == null ? `&nbsp;·&nbsp; ${summary.forearm_side === 'left' ? 'Left' : 'Right'} Forearm (Radius 33%): ${(summary.forearm_t >= 0 ? '+' : '') + summary.forearm_t.toFixed(1)}` : ''}
    </div>
  ...
  <div ${st(`flex-shrink:0;display:flex;align-items:center;gap:16px;padding:12px 16px;border-radius:8px;margin-bottom:14px;background:${P.card};border:1px solid ${P.border}`)}>
    ...
    <div ${st(`font-size:11px;color:${P.gray};line-height:1.7`)}>
      Z-score: <strong>${lowestZVal != null ? (lowestZVal >= 0 ? '+' : '') + lowestZVal.toFixed(1) : '—'}</strong>
      ${summary.spine_z != null ? `&nbsp;·&nbsp; Spine L1–L4: ${(summary.spine_z >= 0 ? '+' : '') + summary.spine_z.toFixed(1)}` : ''}
      ${summary.lowest_hip_z != null ? `&nbsp;·&nbsp; Hip: ${(summary.lowest_hip_z >= 0 ? '+' : '') + summary.lowest_hip_z.toFixed(1)}` : ''}
    </div>
  </div>` : `
  <div ${st(`flex-shrink:0;display:flex;align-items:center;gap:16px;padding:12px 16px;border-radius:8px;margin-bottom:14px;background:${P.card};border:1px solid ${P.border}`)}>
    <div style="flex:1">
