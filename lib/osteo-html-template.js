...
/* ── BMD table ───────────────────────────────────────────────────────────── */
function bmdRow(site, r, isTotal, P, suppressScores = false) {
...
function bmdTable(rows, P, suppressScores = false) {
  ...
  <table style="width:100%;border-collapse:collapse">
    ...
    <tbody>
      ${rows.filter(([, r]) => r != null).map(([site, r, total]) => bmdRow(site, r, total, P, suppressScores)).join('')}
    </tbody>
  </table>
    ...
    <div class="card" ${st(`background:${classBg(summary.overall_class, P)};flex-shrink:0`)}>
      <div class="row" ${st('align-items:flex-end;gap:24px;margin-bottom:10px')}>
        <div ${st('flex:1')}>
          ...
          <div ${st(`font-size:10px;color:${symmetry > 0.5 ? classColor(summary.overall_class) : P.gray};margin-top:4px`)}>
...
function page3(data, P, lh, pageNum, total) {
  ...
  // Z-score age context
  if (!summary.premenopausal && lowestZVal != null) {
    if (lowestZVal >= -1.0) {
      recommendations.push({
        status: 'good',
        title:  'Age-Referenced Bone Density: Within Expected Range',
        body:   `Z-score ${lowestZVal >= 0 ? '+' : ''}${lowestZVal.toFixed(1)} — bone density is reduced compared to young-adult reference values (${classLabel(cls).toLowerCase()} range), while remaining within the expected range for age-matched peers. This pattern is consistent with age-related bone loss rather than accelerated loss.`,
      })
    } else if (lowestZVal <= -2.0) {
      recommendations.push({
        status: 'warn',
        title:  'Age-Referenced Bone Density: Below Expected Range',
        body:   `Z-score ${lowestZVal.toFixed(1)} — bone density is below the expected range for age-matched peers. Secondary causes should be considered: malabsorption (coeliac, IBD), vitamin D deficiency, hypogonadism, hyperparathyroidism, thyroid disease, steroid use, or low body weight.`,
      })
    }
  }
