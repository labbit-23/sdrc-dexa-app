...
// Compact chart matching GE Lunar XPS visual: ~200×138px, horizontal T-score
// zone bands (light fill), single age-declining mean curve, optional Z-score
// sloping reference lines, dual Y-axis (BMD left / T-score integers right).
function spineRefChart(age, bmd, gender, P) {
  ...
  // Compact intrinsic size — matches GE Lunar XPS chart proportions
  const W = 205, H = 140
  const ml = 26, mr = 22, mt = 5, mb = 17
  ...
  return `
  <div ${st('flex-shrink:0;display:flex;flex-direction:column')}>
    <div ${st(`font-size:6.5px;font-weight:600;color:${P.gray};margin-bottom:2px;text-transform:uppercase;letter-spacing:.4px`)}>AP Spine L1–L4${male ? ' · Male ref' : ' · Female ref'}</div>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W}" height="${H}" style="display:block">
      ...
      <!-- Subtle grid -->
      ${xTicks.map(a => `<line x1="${xp(a).toFixed(1)}" y1="${mt}" x2="${xp(a).toFixed(1)}" y2="${mt+cH}" stroke="#ddd" stroke-width="0.4"/>`).join('')}
      ${yTicks.map(b => `<line x1="${ml}" y1="${yp(b).toFixed(1)}" x2="${(ml+cW).toFixed(1)}" y2="${yp(b).toFixed(1)}" stroke="#ddd" stroke-width="0.4"/>`).join('')}

      <!-- T-score zone boundary lines (T=−1 and T=−2.5) -->
      <line x1="${ml}" y1="${yNorm.toFixed(1)}"  x2="${(ml+cW).toFixed(1)}" y2="${yNorm.toFixed(1)}"  stroke="#888" stroke-width="0.8"/>
      <line x1="${ml}" y1="${yOstP.toFixed(1)}" x2="${(ml+cW).toFixed(1)}" y2="${yOstP.toFixed(1)}" stroke="#888" stroke-width="0.8"/>

      <!-- Z-score sloping reference curves (dashed, for age-context) -->
      ...
      <text x="${(ml+cW+12).toFixed(1)}" y="${(mt+2).toFixed(1)}" font-size="5.5" fill="#777" text-anchor="middle" font-weight="600">T</text>
      ${tAxis.map(({t, y}) => `
        <line x1="${(ml+cW).toFixed(1)}" y1="${y.toFixed(1)}" x2="${(ml+cW+2.5).toFixed(1)}" y2="${y.toFixed(1)}" stroke="#aaa" stroke-width="0.6"/>
        <text x="${(ml+cW+4).toFixed(1)}" y="${(y+2).toFixed(1)}" font-size="5.5" fill="#555">${t >= 0 ? '+' : ''}${t}</text>
      `).join('')}
    ...
    <div ${st('display:flex;gap:6px;font-size:6px;color:#999;margin-top:2px;flex-wrap:wrap')}>
      <span>● L1–L4</span>
      <span style="display:inline-flex;align-items:center;gap:2px"><svg width="10" height="3" style="display:block"><line x1="0" y1="1.5" x2="10" y2="1.5" stroke="#1565C0" stroke-width="1.4" opacity="0.7"/></svg>Mean</span>
      <span style="display:inline-flex;align-items:center;gap:2px"><svg width="10" height="3" style="display:block"><line x1="0" y1="1.5" x2="10" y2="1.5" stroke="#888" stroke-width="0.8" stroke-dasharray="4,3"/></svg>T −1/−2.5</span>
    </div>
  </div>`
}
...
<div class="page">
  ...
  <div class="col" ${st('flex:1;gap:12px;min-height:0')}>
    ...
    <!-- ── 3-column main row: image | chart | data ── -->
    <div class="row" ${st('flex:1;gap:14px;min-height:0;align-items:flex-start')}>

      <!-- Col 1: Scan image -->
      <div ${st('flex-shrink:0;display:flex;flex-direction:column;gap:5px;align-items:center;width:140px')}>
      ...
      <!-- Col 2: Reference chart -->
      ${spineTotal ? spineRefChart(pt.age, spineTotal.bmd, pt.gender, P) : ''}

      <!-- Col 3: Key numbers + vertebral table -->
      <div class="col" ${st('flex:1;gap:10px;min-width:0')}>
        <!-- Key numbers card -->
        <div class="card" ${st(`background:${classBg(cls, P)};flex-shrink:0;padding:12px`)}>
          <div ${st('margin-bottom:6px')}>${classBadge(cls)}</div>
          <div ${st('display:flex;align-items:baseline;gap:4px;margin-bottom:2px')}>
            <span ${st(`font-size:36px;font-weight:800;color:${classColor(cls)};line-height:1`)}>${spineTotal?.bmd.toFixed(3) ?? '—'}</span>
            <span ${st(`font-size:10px;color:${P.gray}`)}>g/cm²</span>
          </div>
          ${pYA != null ? `<div ${st(`font-size:9px;color:${P.gray};margin-bottom:8px`)}>% Young Adult: <strong ${st(`color:${P.text}`)}>${pYA}%</strong></div>` : '<div style="margin-bottom:8px"></div>'}
          ${spineT != null ? `
          <div class="row" ${st('gap:16px;margin-bottom:8px')}>
            <div>
              <div class="lbl" ${st('font-size:8px')}>T-Score</div>
              <div ${st(`font-size:26px;font-weight:800;color:${classColor(cls)};line-height:1`)}>${spineT >= 0 ? '+' : ''}${spineT.toFixed(1)}</div>
            </div>
            <div>
              <div class="lbl" ${st('font-size:8px')}>Z-Score</div>
              <div ${st(`font-size:26px;font-weight:800;color:${summary.spine_z != null && summary.spine_z <= -2 ? C.red : C.greenLt};line-height:1`)}>${summary.spine_z != null ? (summary.spine_z >= 0 ? '+' : '') + summary.spine_z.toFixed(1) : '—'}</div>
            </div>
          </div>
          ${tScoreBar(spineT, P)}` : ''}
          ${ageNote}
        ...
        <!-- Vertebral detail table -->
        <div class="card" ${st('flex-shrink:0;padding:10px')}>
          <div class="sec" ${st('font-size:11px;margin-bottom:8px')}>Vertebral Detail</div>
    ...
    ${scanFootnotes([
      'Precision (LSC): ±0.010 g/cm² for AP Spine L1–L4 (68% CI)',
