...
function header(pt, title, P, lh, pageNum, total) {
  ...
  const logoRow = lh ? '' : `
    <div ${st(`display:flex;justify-content:space-between;align-items:center;padding-bottom:8px;margin-bottom:8px;border-bottom:1px solid ${P.border}`)}>
      <img src="https://www.sdrc.in/assets/sdrc-logo-full.png" alt="SDRC Diagnostics" class="sdrc-logo" ${st('height:36px;width:auto;border-radius:4px')}>
  ...
  const inner = lh
    ? `<div ${st('display:flex;justify-content:space-between;align-items:baseline;margin-bottom:2px')}>
        <div ${st(`font-size:20px;font-weight:800;color:${P.text}`)}>${esc(title)}</div>
        ${pgTag}
      </div>
      <div ${st(`font-size:9.5px;color:${P.gray};margin-bottom:4px`)}>
        <span ${st(`font-weight:700;color:${P.text}`)}>${esc(pt.name)}</span>
        &nbsp;·&nbsp; MRN: ${esc(pt.id)} &nbsp;·&nbsp; ${esc(pt.gender)} · ${pt.age}y · ${pt.height_cm} cm · ${pt.weight_kg} kg · Scan ${esc(pt.scan_date)}
      </div>`
...
/* ── Footnotes ───────────────────────────────────────────────────────────── */
function scanFootnotes(lines, P, abs = false) {
  const style = abs
    ? `position:absolute;bottom:0;left:0;right:0;padding-top:6px;border-top:1px solid ${P.border};font-size:8.5px;color:${P.gray};line-height:1.8`
    : `margin-top:auto;padding-top:8px;border-top:1px solid ${P.border};font-size:8.5px;color:${P.gray};line-height:1.8;flex-shrink:0`
  return `
  <div ${st(style)}>
...
function bmdRefChart(age, bmd, site, P) {
  ...
  // GE Lunar DPX normative values back-calculated from SDRC patient data.
  // Spine: peak=1.175, SD=0.117 (verified against two patient data points).
  // Hip: GE Lunar femoral neck runs ~10% above Hologic; Hologic peak ~0.858 → GE ~0.951.
  const [peak, sd] = isSpine ? [1.175, 0.117] : [0.951, 0.102]
  const t0  = peak
  const t1  = peak - sd
  const t2  = peak - 2 * sd
  ...
  // GE Lunar AP spine population mean by age (scaled from NHANES III × 1.121 to match GE Lunar calibration)
  const spineRef = [[20,1.150],[30,1.175],[40,1.138],[50,1.085],[55,1.054],[60,1.004],[65,0.962],[70,0.919],[75,0.877],[80,0.835],[90,0.785]]
...
<div class="page">
  ...
  <div class="col" ${st('flex:1;gap:10px;min-height:0;padding-bottom:28px;position:relative')}>
    ...
    <div class="row" ${st('gap:14px;flex:1;min-height:0;overflow:hidden')}>
      ${hasLeft ? `
      <div ${st('flex:1;display:flex;flex-direction:column;align-items:center;gap:4px;min-height:0')}>
        <div ${st('flex:1;min-height:0;width:100%;display:flex;align-items:center;justify-content:center;max-height:260px')}>
        ...
        <div ${st(`font-size:9px;color:${P.gray};letter-spacing:.4px;flex-shrink:0`)}>Left Femur</div>
        <div ${st(`font-size:8px;color:${P.gray};opacity:.7;letter-spacing:.3px;flex-shrink:0`)}>Image not for diagnosis</div>
      </div>` : ''}
      ${hasRight ? `
      <div ${st('flex:1;display:flex;flex-direction:column;align-items:center;gap:4px;min-height:0')}>
        <div ${st('flex:1;min-height:0;width:100%;display:flex;align-items:center;justify-content:center;max-height:260px')}>
        ...
        <div ${st(`font-size:9px;color:${P.gray};letter-spacing:.4px;flex-shrink:0`)}>Right Femur</div>
        <div ${st(`font-size:8px;color:${P.gray};opacity:.7;letter-spacing:.3px;flex-shrink:0`)}>Image not for diagnosis</div>
      </div>` : ''}
    </div>
...
function compSummaryCard(comp, pt, P) {
  if (!comp) return ''
  const gender = pt?.gender || ''
  const isFemale = !gender || gender.toLowerCase().startsWith('f')
  const fat = comp.fat_pct

  // Zone thresholds matching fatPctBar
  ...
  const agRatio = comp.android_fat_pct != null && comp.gynoid_fat_pct != null && comp.gynoid_fat_pct > 0
    ? comp.android_fat_pct / comp.gynoid_fat_pct : null
  const agThreshold = isFemale ? 0.8 : 1.0
  const centralObesity = agRatio != null && agRatio > agThreshold

  const overallStatus = (fatStatus === 'alert' || centralObesity) ? 'alert'
    : fatStatus === 'warn' ? 'warn' : 'good'
  ...
  const bgColor      = bgColors[overallStatus]

  const findings = []
  if (fat != null)       findings.push(`Body fat <strong>${fat.toFixed(1)}%</strong> — ${fatCategory}`)
  if (agRatio != null)   findings.push(`A/G ratio <strong>${agRatio.toFixed(2)}</strong> — ${centralObesity ? 'Central fat distribution' : 'Normal distribution'}`)

  let recommendation, timing
  if (fatCategory === 'Obese' || centralObesity) {
    recommendation = 'Elevated body fat and/or central adiposity detected from estimated values. A dedicated Whole Body DXA scan is strongly recommended to accurately quantify fat mass, lean mass, and regional distribution (FMI, LMI), and to establish a baseline for monitoring.'
    timing = 'Whole Body DXA recommended within 3 months'
  } else if (fatCategory === 'High') {
    recommendation = 'Body fat percentage is in the elevated range. A Whole Body DXA scan would provide precise fat and lean mass quantification — including fat mass index (FMI), lean mass index (LMI), and regional detail — that is not available from this AP Spine + Femur scan.'
    timing = 'Whole Body DXA recommended within 6 months'
  } else if (comp.is_estimated) {
    recommendation = 'Estimated body composition from AP Spine + Femur analysis — not a true body composition measurement. A dedicated Whole Body DXA would provide accurate fat mass, lean mass, and regional body composition data for a complete metabolic picture.'
  ...
  return `
  <div ${st(`background:${bgColor};border:1px solid ${borderColor}40;border-left:4px solid ${borderColor};border-radius:6px;padding:11px 16px;margin-bottom:12px`)}>
    <div ${st(`font-size:11px;font-weight:700;color:${borderColor};margin-bottom:5px`)}>Body Composition Summary</div>
    <div ${st(`font-size:10px;color:${P.text};line-height:1.7;margin-bottom:5px`)}>
      ${findings.map(f => `<span ${st(`margin-right:16px`)}>${f}</span>`).join('')}
    </div>
    <div ${st(`font-size:10px;color:${P.grayLt};line-height:1.65`)}>${recommendation}</div>
    ${timing ? `<div ${st(`margin-top:6px;font-size:10px;font-weight:700;color:${borderColor}`)}>${timing}</div>` : ''}
  </div>`
}
  ...
  <div ${st(`flex-shrink:0;margin-top:10px;background:${P.card};border:1px solid ${P.border};border-radius:6px;padding:10px 14px`)}>
    <div class="row" ${st('gap:20px;align-items:flex-start')}>
      <div ${st('flex:1')}>
        ...
        <div ${st(`margin-top:3px;font-size:9px;color:${P.gray}`)}>ISCD: Only Femoral Neck &amp; Total Hip used for WHO classification — Trochanter and Ward's are not diagnostic sites.</div>
      </div>
...
export function generateOsteoHtml(data, { dark = false, letterhead = false, history = [], preview = false } = {}) {
  ...
  // Base pages (spine + optional hip)
  const basePages = hasFemur ? [page1, page2] : [page1]
...
@media print{
  ...
  .page{height:${letterhead ? '211mm' : '297mm'}!important;overflow:hidden!important;transform:none!important;margin:0!important;box-shadow:none!important;page-break-after:always;display:flex}
  .page:last-child{page-break-after:auto}
}
</style></head><body>
${pagesHtml}
