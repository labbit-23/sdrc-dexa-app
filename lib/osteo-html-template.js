...
function pageForearm(data, P, lh, pageNum, total) {
  ...
  const rows = fa.rows.map(({ region, left, right }) => `
    <tr>
      <td ${st(`padding:8px;font-size:11px;color:${P.text};font-weight:600`)}>${region}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.text}`)}>${fmtBmd(left?.bmd)}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.gray}`)}>${fmt(left?.T)}</td>
      ...
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.gray}`)}>${fmt(right?.T)}</td>
