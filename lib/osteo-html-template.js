...
function pageForearm(data, P, lh, pageNum, total) {
  const { patient: pt, forearm_summary: fa } = data
  if (!fa) return ''
  ...
  const rows = fa.rows.map(({ region, isDiagnostic, left, right }) => {
    const lTColor = tScoreColor(left?.T)
    const rTColor = tScoreColor(right?.T)
    ...
    return `<tr style="${rowBg}">
      ...
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.text}`)}>${fmtBmd(left?.bmd)}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${lTColor};font-weight:700`)}>${fmt(left?.T)}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.gray}`)}>${fmt(left?.Z)}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.text}`)}>${fmtBmd(right?.bmd)}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${rTColor};font-weight:700`)}>${fmt(right?.T)}</td>
      <td ${st(`padding:8px;font-size:11px;text-align:right;color:${P.gray}`)}>${fmt(right?.Z)}</td>
    </tr>`
  }).join('')
...
<div class="page">
  ...
  <div class="col" ${st('flex:1;gap:12px;min-height:0')}>
    <div class="card" ${st(`background:${classBg(primaryClass, P)};flex-shrink:0`)}>
      <div class="row" ${st('align-items:flex-end;gap:24px;margin-bottom:10px')}>
        ...
        <div ${st('display:flex;flex-direction:column')}>
          ...
          <div ${st(`font-size:9px;color:${P.gray};margin-top:2px`)}>Diagnostic site</div>
          <div ${st('flex:1;min-height:8px')}></div>
    ...
    <div class="card" ${st('flex-shrink:0')}>
      <div class="sec">Forearm Bone Density — All Measurements</div>
      <table ${st('width:100%;border-collapse:collapse;margin-top:12px')}>
        <thead><tr>
          <th ${st(`padding:8px;font-size:10px;font-weight:700;color:${P.gray};text-align:left;border-bottom:2px solid ${P.border}`)}></th>
          <th colspan="3" ${st(`padding:8px;font-size:10px;font-weight:700;color:${P.gray};text-align:center;border-bottom:2px solid ${P.border}`)}><strong>Left</strong></th>
          <th colspan="3" ${st(`padding:8px;font-size:10px;font-weight:700;color:${P.gray};text-align:center;border-bottom:2px solid ${P.border}`)}><strong>Right</strong></th>
        </tr><tr>
          <th ${st(`padding:8px;font-size:9px;color:${P.gray};text-align:left;border-bottom:1px solid ${P.border}`)}}></th>
          <th ${st(`padding:8px;font-size:9px;color:${P.gray};text-align:right;border-bottom:1px solid ${P.border}`)}}> BMD</th>
          <th ${st(`padding:8px;font-size:9px;color:${P.gray};text-align:right;border-bottom:1px solid ${P.border}`)}}>T</th>
          <th ${st(`padding:8px;font-size:9px;color:${P.gray};text-align:right;border-bottom:1px solid ${P.border}`)}}>Z</th>
          <th ${st(`padding:8px;font-size:9px;color:${P.gray};text-align:right;border-bottom:1px solid ${P.border}`)}}> BMD</th>
          <th ${st(`padding:8px;font-size:9px;color:${P.gray};text-align:right;border-bottom:1px solid ${P.border}`)}}>T</th>
          <th ${st(`padding:8px;font-size:9px;color:${P.gray};text-align:right;border-bottom:1px solid ${P.border}`)}}>Z</th>
        </tr></thead>
      ...
      <div ${st(`font-size:8px;color:${P.gray};margin-top:10px`)}}><strong>Note:</strong> T & Z scores available for Radius sites only. Reference: Ulna Total, Both Total (no scores).</div>
    </div>
  </div>
</div>`
}
...
export function generateOsteoHtml(data, { dark = false, letterhead = false, history = [], preview = false } = {}) {
  ...
  const hasForearm = Object.keys(data.left_forearm ?? {}).length > 0
    || Object.keys(data.right_forearm ?? {}).length > 0
