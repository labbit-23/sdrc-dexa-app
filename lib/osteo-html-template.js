...
function pageForearm(data, P, lh, pageNum, total) {
  ...
  const hasImages = images && (images.left_forearm_url || images.right_forearm_url)
...
<div class="page">
  ...
  <div class="col" ${st('flex:1;gap:12px;min-height:0')}>
    <div class="card" ${st(`background:${classBg(primaryClass, P)};flex-shrink:0`)}>
      <div class="row" ${st('align-items:flex-end;gap:24px;margin-bottom:10px')}>
        <div ${st('flex:1;display:flex;flex-direction:column')}>
          ...
          <div ${st(`font-size:11px;color:${P.gray};margin-top:8px;line-height:1.6`)}>T-score: <strong ${st(`color:${primaryColor}`)}>${fmt(primaryT)}</strong></div>
        </div>
        ${primaryT != null ? `
        <div ${st('display:flex;flex-direction:column')}>
          <div class="lbl">Radius 33% BMD</div>
          <div ${st(`font-size:9px;color:${P.gray};margin-top:2px`)}>ISCD diagnostic site</div>
          <div ${st('flex:1;min-height:8px')}></div>
          <div ${st(`font-size:32px;font-weight:800;color:${primaryColor};line-height:1`)}>${fmt(primaryT)}</div>
        </div>` : ''}
      </div>
    </div>
    <div class="card" ${st('flex-shrink:0')}>
      <div class="sec">Forearm Measurements</div>
      <div ${st(`font-size:8px;color:${P.gray};margin-top:8px;margin-bottom:12px;line-height:1.5`)}}><strong>ISCD:</strong> Radius 33% (one-third distal) is the standard diagnostic site for forearm BMD. T & Z scores provided for Radius sites (UD, 33%, Total); reference values shown for Ulna and Both.</div>
      <div ${st('display:flex;flex-direction:column;gap:16px')}>
        ${hasLeft ? `
        <div ${st('border-top:1px solid ' + P.border + ';padding-top:12px')}>
          <div class="lbl" ${st('font-size:10px;margin-bottom:8px;color:' + P.gray)}>Left Forearm</div>
        ...
        ${hasRight ? `
        <div ${st('border-top:1px solid ' + P.border + ';padding-top:12px')}>
          <div class="lbl" ${st('font-size:10px;margin-bottom:8px;color:' + P.gray)}>Right Forearm</div>
          <table ${st('width:100%;border-collapse:collapse;font-size:10px')}>
            <thead><tr>
              <th ${st(`padding:6px;text-align:left;border-bottom:1px solid ${P.border};color:${P.gray}`)}></th>
              <th ${st(`padding:6px;text-align:right;border-bottom:1px solid ${P.border};color:${P.gray}`)}}> BMD</th>
    ...
    ${hasImages ? `
    <div class="card" ${st('flex-shrink:0')}>
      <div class="sec">Forearm Images</div>
      <div ${st('display:flex;flex-direction:column;gap:16px;margin-top:12px')}>
        ${hasLeft && images.left_forearm_url ? `
        <div ${st('display:flex;flex-direction:column;gap:4px;align-items:center')}>
          <img src="${images.left_forearm_url}" alt="Left forearm"
               ${st('width:100%;max-width:300px;height:auto;border-radius:6px;display:block;object-fit:contain')}>
          <div ${st(`font-size:9px;color:${P.gray};text-align:center`)}>Left Forearm</div>
          <div ${st(`font-size:7.5px;color:${P.gray};opacity:.6;text-align:center`)}>Image not for diagnosis</div>
        </div>` : ''}
        ${hasRight && images.right_forearm_url ? `
        <div ${st('display:flex;flex-direction:column;gap:4px;align-items:center')}>
          <img src="${images.right_forearm_url}" alt="Right forearm"
               ${st('width:100%;max-width:300px;height:auto;border-radius:6px;display:block;object-fit:contain')}>
          <div ${st(`font-size:9px;color:${P.gray};text-align:center`)}>Right Forearm</div>
          <div ${st(`font-size:7.5px;color:${P.gray};opacity:.6;text-align:center`)}>Image not for diagnosis</div>
        </div>` : ''}
      </div>
    </div>` : ''}
...
function page3(data, P, lh, pageNum, total) {
  ...
  const hasValidTScores = summary.lowest_hip_t != null || summary.spine_t != null
  const hasValidZScores = summary.lowest_hip_z != null || summary.spine_z != null
