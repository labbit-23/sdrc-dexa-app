...
function bmdRefChart(age, bmd, site, P) {
  ...
  const neckRef  = [[20,0.855],[30,0.858],[40,0.838],[50,0.798],[55,0.763],[60,0.728],[65,0.696],[70,0.665],[75,0.632],[80,0.600],[90,0.555]]
  ...
  const totalW = W + 40
  return `
  <div ${st('margin-top:10px')}>
    <div ${st(`font-size:8px;font-weight:600;color:${P.gray};margin-bottom:3px;text-transform:uppercase;letter-spacing:.5px`)}>Densitometry Reference — ${isSpine ? 'AP Spine L1–L4' : 'Femoral Neck'}</div>
...
<div class="page">
  ...
  <div class="col" ${st('flex:1;gap:14px;min-height:0')}>
    ...
    <div class="row" ${st('gap:14px;flex-shrink:0')}>
      ...
      <div ${st('flex:1;display:flex;flex-direction:column;align-items:center;gap:4px')}>
        <img src="${images.left_femur_overlay_url || images.left_femur_url}" alt="Left femur"
             ${st('max-height:200px;max-width:100%;width:auto;height:auto;display:block;border-radius:8px')}
             onerror="this.style.display='none'">
    ...
    <div class="card" ${st('flex-shrink:0')}>
      ...
      <div class="row" ${st('gap:14px')}>
        <div ${st(`flex:1;background:${P.bg};border-radius:6px;padding:10px 14px;font-size:10px;color:${P.gray};line-height:1.7`)}>
...
function fatPctBar(fatPct, gender, P) {
  ...
  // Body fat % zones (ACSM / Gallagher et al.)
  // Female: <21 Athletic, 21-33 Normal, 33-39 High, >39 Obese
  // Male:   <14 Athletic, 14-25 Normal, 25-31 High, >31 Obese
  ...
  const zonesBars = zones.map(z => `
    <div ${st(`width:${z.w}%;background:${z.color};display:flex;align-items:center;justify-content:center`)}>
      <span ${st('font-size:7px;color:#fff;font-weight:700')}>${z.label}</span>
    </div>`).join('')
  const ticks = isFemale ? ['0%', '21%', '33%', '39%', '50%'] : ['0%', '14%', '25%', '31%', '50%']
  return `
  <div ${st('position:relative;margin:10px 0 4px')}>
    <div ${st('display:flex;height:18px;border-radius:4px;overflow:hidden')}>${zonesBars}</div>
    <div ${st(`position:absolute;left:${markerPct}%;top:-4px;width:3px;height:26px;background:${P.text};border-radius:2px;transform:translateX(-50%);box-shadow:0 0 3px rgba(0,0,0,.4)`)}></div>
  </div>
  <div ${st(`display:flex;justify-content:space-between;font-size:8px;color:${P.gray};margin-bottom:8px`)}>
...
function agBar(agRatio, gender, P) {
  ...
  const riskLabel = risk ? 'Central Obesity Pattern' : 'Normal Fat Distribution'
  return `
  <div ${st('display:flex;align-items:center;gap:10px;margin-top:4px')}>
    <div ${st(`font-size:9px;color:${P.gray};white-space:nowrap`)}>A/G Ratio</div>
    <div ${st('flex:1;position:relative')}>
      <div ${st(`display:flex;height:14px;border-radius:3px;overflow:hidden`)}>
        <div ${st(`width:${isFemale ? 53 : 50}%;background:${C.greenLt}`)}></div>
        <div ${st(`width:${isFemale ? 47 : 50}%;background:${C.red}`)}></div>
      </div>
      <div ${st(`position:absolute;left:${Math.min(parseFloat(agRatio) / 1.6 * 100, 99)}%;top:-3px;width:3px;height:20px;background:${P.text};border-radius:2px;transform:translateX(-50%)`)}></div>
    </div>
    <div ${st(`font-size:22px;font-weight:800;color:${riskColor};line-height:1`)}>${agRatio}</div>
    <div ${st(`font-size:9px;font-weight:700;color:${riskColor}`)}>${riskLabel}</div>
  </div>
  <div ${st(`font-size:8px;color:${P.gray};margin-top:3px`)}>Threshold: &gt;${threshold} (${isFemale ? 'female' : 'male'}) indicates android/central adiposity risk</div>`
}
...
function compCard(comp, pt, P) {
  ...
  <div ${st(`background:${P.card};border:1px solid ${P.border};border-radius:8px;padding:12px 16px;margin-bottom:12px`)}>
    ...
    <div ${st('display:grid;grid-template-columns:repeat(4,1fr);gap:6px 12px;margin-bottom:10px')}>
      ${statItems.map(it => `
        <div>
          <div ${st(`font-size:9px;color:${P.gray}`)}>${it.label}</div>
          <div ${st(`font-size:13px;font-weight:700;color:${P.text}`)}>${it.val}${it.unit ? `<span ${st(`font-size:9px;font-weight:400;color:${P.gray};margin-left:2px`)}>${it.unit}</span>` : ''}</div>
        </div>`).join('')}
    </div>
