...
function bmdTable(rows, P, suppressScores = false, hideYA = false, compact = false) {
  ...
  <div style="font-size:9px;color:${P.gray};margin-top:6px">
    BMD: <span style="color:${C.greenLt}">■</span> &gt;0.9 · <span style="color:${C.amber}">■</span> 0.7–0.9 · <span style="color:${C.red}">■</span> &lt;0.7 g/cm²${suppressScores ? '' : ` &nbsp;·&nbsp;
    T: <span style="color:${C.greenLt}">■</span> ≥−1.0 · <span style="color:${C.amber}">■</span> −1 to −2.5 · <span style="color:${C.red}">■</span> ≤−2.5`}
  </div>`
}
...
/* ── Spine BMD Reference Chart (GE Lunar style) ─────────────────────────── */
// Single L1-L4 dot. Zone coloring follows age-matched Z-score curves (sloping
// polygons) with T-score horizontal dashed lines overlaid. Right Y-axis shows
// T-score integers so doctors can read classification at a glance.
function spineRefChart(age, bmd, gender, P) {
  if (age == null || bmd == null || bmd === 0) return ''
  const sRef = _bmNorm && _bmNorm.spine
  if (!sRef) return ''
  const male = (gender || '').toLowerCase().startsWith('m')
  const { peak, sd, curve } = male ? sRef.male : sRef.female

  // Intrinsic SVG size (rendered responsively via width:100%)
  const W = 340, H = 158
  const ml = 32, mr = 28, mt = 6, mb = 20
  const cW = W - ml - mr, cH = H - mt - mb

  const AGE_MIN = 20, AGE_MAX = 90
  const BMD_MIN = 0.54, BMD_MAX = 1.50
  ...
  // T-score WHO thresholds (horizontal)
  const tOst  = peak - sd         // T = −1
  const tOstP = peak - 2.5 * sd   // T = −2.5

  // Age-following Z-score reference curves
  function curvePts(kSD) {
    return curve.map(([a, b]) => [xp(a), yp(b - kSD * sd)])
  }
  const c0  = curve.map(([a, b]) => [xp(a), yp(b)])
  const c1  = curvePts(1)
  const c25 = curvePts(2.5)

  const pts = arr => arr.map(([x, y]) => `${x.toFixed(1)},${cy(y).toFixed(1)}`).join(' ')

  // Zone polygons (Z-score age-following — primary zone coloring)
  const polyNormal  = `${ml},${mt} ${(ml+cW).toFixed(1)},${mt} ${pts([...c1].reverse())}`
  const polyOstopen = `${pts(c1)} ${pts([...c25].reverse())}`
  const polyOsteo   = `${pts(c25)} ${(ml+cW).toFixed(1)},${mt+cH} ${ml},${mt+cH}`

  // Right-axis T-score integers
  const tAxis = []
  for (let t = 3; t >= -6; t--) {
    const bAtT = peak + t * sd
    if (bAtT >= BMD_MIN && bAtT <= BMD_MAX) tAxis.push({ t, y: cy(yp(bAtT)) })
  }

  // Zone label positions: midpoint of each polygon at age ~60
  const iMid = c1.findIndex(([x]) => x >= xp(58))
  const idx   = (iMid >= 0 && iMid < c1.length) ? iMid : Math.floor(c1.length * 0.6)
  const ym1   = cy(c1[idx][1])
  const ym25  = cy(c25[idx][1])
  const xLbl  = (c1[idx][0] + 8).toFixed(1)
  const yLblN = ((mt + ym1)     / 2 + 2).toFixed(1)
  const yLblO = ((ym1 + ym25)   / 2 + 2).toFixed(1)
  const yLblP = ((ym25 + mt+cH) / 2 + 2).toFixed(1)

  // Patient dot — colored by T-score zone
  const dotX = Math.max(ml + 6, Math.min(ml + cW - 6, xp(age)))
  const dotY = Math.max(mt + 6, Math.min(mt + cH - 6, yp(bmd)))
  const dotColor = bmd <= tOstP ? C.red : bmd <= tOst ? C.amber : C.greenLt
  const lblY = dotY > mt + 14 ? (dotY - 10).toFixed(1) : (dotY + 18).toFixed(1)
  ...
  return `
  <div ${st('flex:1;min-width:0;display:flex;flex-direction:column')}>
    <div ${st(`font-size:7px;font-weight:600;color:${P.gray};margin-bottom:2px;text-transform:uppercase;letter-spacing:.5px`)}>
      AP Spine Reference — L1–L4${male ? ' · Male' : ' · Female'} · Age-matched zones + T-score thresholds
    </div>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="100%" style="display:block;flex:1">
      <defs><clipPath id="${clipId}"><rect x="${ml}" y="${mt}" width="${cW}" height="${cH}"/></clipPath></defs>
      <rect x="${ml}" y="${mt}" width="${cW}" height="${cH}" fill="#f8fafc"/>

      <!-- Zone polygons: Z-score age-following (GE Lunar style) -->
      <polygon points="${polyNormal}"  fill="#4CAF5038" clip-path="url(#${clipId})"/>
      <polygon points="${polyOstopen}" fill="#FF980045" clip-path="url(#${clipId})"/>
      <polygon points="${polyOsteo}"   fill="#F4433632" clip-path="url(#${clipId})"/>

      <!-- Grid -->
      ${xTicks.map(a => `<line x1="${xp(a).toFixed(1)}" y1="${mt}" x2="${xp(a).toFixed(1)}" y2="${mt+cH}" stroke="#e0e0e0" stroke-width="0.4"/>`).join('')}
      ${yTicks.map(b => `<line x1="${ml}" y1="${yp(b).toFixed(1)}" x2="${(ml+cW).toFixed(1)}" y2="${yp(b).toFixed(1)}" stroke="#e0e0e0" stroke-width="0.4"/>`).join('')}

      <!-- T-score horizontal dashed reference lines -->
      <line x1="${ml}" y1="${yp(tOst).toFixed(1)}"  x2="${(ml+cW).toFixed(1)}" y2="${yp(tOst).toFixed(1)}"  stroke="#555" stroke-width="0.9" stroke-dasharray="5,3" opacity="0.75"/>
      <line x1="${ml}" y1="${yp(tOstP).toFixed(1)}" x2="${(ml+cW).toFixed(1)}" y2="${yp(tOstP).toFixed(1)}" stroke="#555" stroke-width="0.9" stroke-dasharray="5,3" opacity="0.75"/>

      <!-- Age-following Z-score reference curves (lines only) -->
      <polyline points="${pts(c25)}" fill="none" stroke="#B71C1C" stroke-width="1"   stroke-dasharray="5,3" opacity="0.6" clip-path="url(#${clipId})"/>
      <polyline points="${pts(c1)}"  fill="none" stroke="#B76E00" stroke-width="1"   stroke-dasharray="5,3" opacity="0.6" clip-path="url(#${clipId})"/>
      <polyline points="${pts(c0)}"  fill="none" stroke="#1565C0" stroke-width="1.6" opacity="0.6"           clip-path="url(#${clipId})"/>

      <!-- Chart border -->
      <rect x="${ml}" y="${mt}" width="${cW}" height="${cH}" fill="none" stroke="#bbb" stroke-width="0.8"/>

      <!-- Zone labels inside polygons -->
      <text x="${xLbl}" y="${yLblN}" font-size="7" fill="#2E7D32" font-weight="700" opacity="0.85">Normal</text>
      <text x="${xLbl}" y="${yLblO}" font-size="7" fill="#B76E00" font-weight="700" opacity="0.85">Osteopenia</text>
      <text x="${xLbl}" y="${yLblP}" font-size="7" fill="#B71C1C" font-weight="700" opacity="0.85">Osteoporosis</text>

      <!-- Left Y-axis: BMD labels -->
      ${yTicks.map(b => `<text x="${(ml-3).toFixed(1)}" y="${(yp(b)+2.5).toFixed(1)}" text-anchor="end" font-size="6" fill="#666">${b.toFixed(1)}</text>`).join('')}
      <text x="7" y="${(mt+cH/2).toFixed(1)}" text-anchor="middle" font-size="6.5" fill="#666" transform="rotate(-90,7,${(mt+cH/2).toFixed(1)})">BMD g/cm²</text>

      <!-- Right Y-axis: T-score integers -->
      <text x="${(ml+cW+14).toFixed(1)}" y="${(mt-1).toFixed(1)}" font-size="6" fill="#666" text-anchor="middle" font-weight="700">T</text>
      ${tAxis.map(({t, y}) => `
        <line x1="${(ml+cW).toFixed(1)}" y1="${y.toFixed(1)}" x2="${(ml+cW+3).toFixed(1)}" y2="${y.toFixed(1)}" stroke="#999" stroke-width="0.7"/>
        <text x="${(ml+cW+5).toFixed(1)}" y="${(y+2.5).toFixed(1)}" font-size="6" fill="#555">${t >= 0 ? '+' : ''}${t}</text>
      `).join('')}

      <!-- X-axis: Age labels -->
      ${xTicks.map(a => `<text x="${xp(a).toFixed(1)}" y="${(mt+cH+10).toFixed(1)}" text-anchor="middle" font-size="6" fill="#888">${a}</text>`).join('')}
      <text x="${(ml+cW/2).toFixed(1)}" y="${H-2}" text-anchor="middle" font-size="6.5" fill="#666">Age (years)</text>

      <!-- Patient dot (L1–L4) -->
      <circle cx="${dotX.toFixed(1)}" cy="${dotY.toFixed(1)}" r="6.5" fill="white" stroke="${dotColor}" stroke-width="2.5"/>
      <circle cx="${dotX.toFixed(1)}" cy="${dotY.toFixed(1)}" r="3"   fill="${dotColor}"/>
      <text x="${dotX.toFixed(1)}" y="${lblY}" text-anchor="middle" font-size="7.5" fill="${dotColor}" font-weight="800">${bmd.toFixed(3)}</text>
    </svg>
    <div ${st('display:flex;gap:8px;font-size:6.5px;color:#888;margin-top:3px;flex-wrap:wrap')}>
      <span>● L1–L4 patient BMD</span>
      <span style="display:inline-flex;align-items:center;gap:2px"><svg width="14" height="3" style="display:block"><line x1="0" y1="1.5" x2="14" y2="1.5" stroke="#1565C0" stroke-width="1.5" opacity="0.6"/></svg>Age-matched mean</span>
      <span style="display:inline-flex;align-items:center;gap:2px"><svg width="14" height="3" style="display:block"><line x1="0" y1="1.5" x2="14" y2="1.5" stroke="#555" stroke-width="0.9" stroke-dasharray="5,3" opacity="0.75"/></svg>T=−1 / −2.5</span>
      <span style="display:inline-flex;align-items:center;gap:2px"><svg width="14" height="3" style="display:block"><line x1="0" y1="1.5" x2="14" y2="1.5" stroke="#B76E00" stroke-width="1" stroke-dasharray="5,3" opacity="0.7"/></svg>Z-score boundaries</span>
    </div>
  </div>`
}
...
function bmdRefChart(age, bmd, site, gender, P, dotLeft = null, dotRight = null) {
  ...
  <div ${st('flex-shrink:0')}>
    ...
    <div ${st('display:flex;align-items:center;flex-wrap:nowrap;gap:5px;font-size:6.5px;margin-top:3px;color:#666;overflow:hidden')}>
      ${twoSided
        ? `<span style="white-space:nowrap;color:#0D7377">● L neck</span>
           <span style="white-space:nowrap;color:#7B2D8B">● R neck</span>`
        : `<span style="white-space:nowrap">● Patient</span>`}
      <span style="display:inline-flex;align-items:center;gap:2px;white-space:nowrap"><svg width="12" height="4" style="display:block"><line x1="0" y1="2" x2="12" y2="2" stroke="#1565C0" stroke-width="1.5" opacity="0.75"/></svg>${isSpine ? `Pop. mean (${male ? 'male' : 'female'})` : `Pop. mean · zones: NHANES ♀`}</span>
...
function page1(data, P, lh, pageNum, total) {
  ...
  // %Young Adult for L1-L4: prefer MDB stored value, fall back to computed from peak
  ...
  const ageNote = summary.premenopausal
    ? `<div ${st(`font-size:9.5px;color:${C.amber};margin-top:6px;line-height:1.6;border-top:1px solid ${P.border};padding-top:6px`)}>
        ⚠ Premenopausal woman (&lt;50 yrs) — Z-score preferred over T-score. Z ≤−2.0 = below expected range for age.
      </div>` : ''
...
<div class="page">
  ...
  <div class="col" ${st('flex:1;gap:12px;min-height:0')}>
    ...
    <!-- ── Hero card: key numbers ── -->
    <div class="card" ${st(`background:${classBg(cls, P)};flex-shrink:0`)}>
      <div class="row" ${st('align-items:flex-end;gap:20px;margin-bottom:10px')}>
        <div ${st('flex:1;display:flex;flex-direction:column')}>
          ...
          <div ${st('margin-top:6px')}>${classBadge(cls)}</div>
          <div ${st('display:flex;align-items:baseline;gap:5px;margin-top:10px')}>
          ...
          ${pYA != null ? `<div ${st(`font-size:10px;color:${P.gray};margin-top:3px`)}>% Young Adult: <strong ${st(`color:${P.text}`)}>${pYA}%</strong></div>` : ''}
        </div>
        ${spineT != null ? `
        <div ${st('display:flex;flex-direction:column;align-items:center')}>
          <div class="lbl">T-Score</div>
          <div ${st(`font-size:9px;color:${P.gray};margin-top:2px`)}>vs peak (age 30)</div>
        ...
        <div ${st('display:flex;flex-direction:column;align-items:center')}>
          ...
          <div ${st(`font-size:9px;color:${P.gray};margin-top:2px`)}>vs age peers</div>
          <div ${st('flex:1;min-height:8px')}></div>
    ...
    <!-- ── Image + Chart side by side ── -->
    <div class="row" ${st('gap:12px;flex:1;min-height:0;align-items:stretch')}>
      <!-- Scan image -->
      <div ${st('flex-shrink:0;display:flex;flex-direction:column;gap:4px;align-items:center;justify-content:center;width:145px')}>
        <img src="${images.spine_overlay_url || images.spine_url}" alt="Spine scan"
             ${st('width:100%;height:auto;border-radius:8px;display:block;object-fit:contain')}
        ...
        <div ${st(`font-size:8px;color:${P.gray};opacity:.65;text-align:center`)}>Image not for diagnosis</div>
      </div>
      <!-- Reference chart fills remaining space -->
      ${spineTotal ? spineRefChart(pt.age, spineTotal.bmd, pt.gender, P) : ''}
    </div>

    <!-- ── Vertebral detail table — full width ── -->
    <div class="card" ${st('flex-shrink:0')}>
      <div class="sec">L1–L4 Vertebral Detail</div>
      ${bmdTable(spineRows, P, false, true)}
    </div>
  ...
  <div class="col" ${st('flex:1;gap:10px;min-height:0')}>
    <div class="card" ${st(`background:${classBg(hipClass ?? summary.overall_class, P)};flex-shrink:0`)}>
      ...
      <div ${st('display:flex;gap:16px;align-items:flex-start')}>
        ...
        ${(hasLeft || hasRight) ? bmdRefChart(pt.age, null, 'hip', pt.gender, P, lf.Neck?.bmd ?? null, rf.Neck?.bmd ?? null) : ''}
      </div>
    </div>
    ${hasLeft ? `
    <div class="row" ${st('gap:14px;flex-shrink:0;align-items:flex-start')}>
      ...
      <div class="card" ${st(`flex:1${summary.left_implant ? `;background:${P.prosthesisBg};border-color:${C.blue}44` : ''}`)}>
        ...
        <div ${st(`font-size:14px;font-weight:700;color:${C.blueLt};margin:10px 0 12px`)}>Excluded from analysis</div>
        <div ${st(`font-size:9.5px;color:${P.gray};line-height:1.7;border-top:1px solid ${C.blue}44;padding-top:10px`)}>
          <span ${st(`font-weight:700;color:${C.blueLt}`)}>Recommendation:</span>
          Dedicated Orthopedic Hip / Prosthetic Hip DXA study may be considered if periprosthetic bone assessment is clinically required.
        </div>` : bmdTable(lfRows, P)}
      </div>
    </div>` : ''}
    ${hasRight ? `
    <div class="row" ${st('gap:14px;flex-shrink:0;align-items:flex-start')}>
