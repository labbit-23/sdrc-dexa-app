...
function compCard(comp, pt, P) {
  ...
  return `
  <div ${st(`background:${P.card};border:1px solid ${P.border};border-left:4px solid ${borderColor};border-radius:8px;padding:10px 14px;margin-bottom:10px`)}>
    ...
    <div ${st('display:grid;grid-template-columns:1fr 1fr;gap:8px 16px;margin-bottom:6px')}>
      ...
      <div>
        <div ${st('display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-bottom:5px')}>
    ...
    <div ${st(`border-top:1px solid ${P.border};padding-top:6px`)}>
      <div ${st(`font-size:8.5px;color:${P.text};margin-bottom:2px`)}>
        ${findings.map(f => `<span ${st('margin-right:14px')}>${f}</span>`).join('')}
      </div>
...
<div class="page">
  ...
  <div ${st(`flex-shrink:0;margin-top:10px;background:${P.card};border:1px solid ${P.border};border-radius:6px;padding:7px 12px`)}>
    <div class="row" ${st('gap:16px;align-items:flex-start')}>
      <div ${st('flex:1')}>
        <span ${st(`font-size:8.5px;font-weight:700;color:${C.tealLt}`)}>T-Score</span>
        <span ${st(`font-size:8px;color:${P.gray};margin-left:5px`)}>vs healthy 30-year-old · WHO standard for bone loss diagnosis</span>
        <div ${st('margin-top:3px')}>
          <span ${st(`font-size:8px;color:${C.greenLt};font-weight:600`)}>≥ −1.0 Normal</span>
          <span ${st(`font-size:8px;color:${P.gray}`)}> · </span>
          <span ${st(`font-size:8px;color:${C.amber};font-weight:600`)}>−1 to −2.5 Osteopenia</span>
          <span ${st(`font-size:8px;color:${P.gray}`)}> · </span>
          <span ${st(`font-size:8px;color:${C.red};font-weight:600`)}>≤ −2.5 Osteoporosis</span>
        </div>
      </div>
      <div ${st(`width:1px;background:${P.border};align-self:stretch`)}></div>
      <div ${st('flex:1')}>
        <span ${st(`font-size:8.5px;font-weight:700;color:${C.tealLt}`)}>Z-Score</span>
        <span ${st(`font-size:8px;color:${P.gray};margin-left:5px`)}>vs same age &amp; sex · below −2.0 = "below expected for age"</span>
        <div ${st(`margin-top:3px;font-size:8px;color:${P.gray};line-height:1.5`)}>If T is low but Z is near 0, bone loss is age-related, not accelerated. Z ≤ −2.0 warrants secondary cause investigation regardless of T-score.</div>
      </div>
    </div>
  </div>
