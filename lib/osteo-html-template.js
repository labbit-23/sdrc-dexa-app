...
function bmdTable(rows, P, suppressScores = false, hideYA = false, compact = false) {
  ...
  <div style="font-size:8.5px;color:${P.gray};margin-top:6px;line-height:1.6">
    <div><strong>T-Score</strong> vs healthy 30-year-old (WHO standard for bone loss diagnosis): <span style="color:${C.greenLt}">≥−1.0 Normal</span> · <span style="color:${C.amber}">−1 to −2.5 Osteopenia</span> · <span style="color:${C.red}">≤−2.5 Osteoporosis</span></div>
    ${suppressScores ? '' : `<div><strong>Z-Score</strong> vs same age &amp; sex peers: ≤−2.0 indicates "below expected for age." If T is low but Z is near 0, bone loss is age-related. Z ≤−2.0 warrants secondary cause investigation regardless of T-score.</div>`}
  </div>`
}
