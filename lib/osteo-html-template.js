  ...
  <div class="col" ${st('flex:1;gap:10px;min-height:0')}>
    ...
    <div class="row" ${st('gap:14px;flex-shrink:0;align-items:flex-start')}>
      ...
      <div class="card" ${st(`flex:1${summary.left_implant ? `;background:${P.prosthesisBg};border-color:${C.blue}44` : ''}`)}>
        ...
        ${summary.left_implant ? `<div ${st(`font-size:10px;color:${C.blueLt};margin-bottom:8px;line-height:1.5;border-left:3px solid ${C.blue};padding-left:8px`)}>Hip prosthesis — BMD not applicable. Values below reflect prosthesis artifact and are excluded from bone density classification.</div>` : ''}
        ${bmdTable(lfRows, P, summary.left_implant)}
      </div>
    </div>` : ''}
    ${hasRight ? `
    <div class="row" ${st('gap:14px;flex-shrink:0;align-items:flex-start')}>
      ...
      <div class="card" ${st(`flex:1${summary.right_implant ? `;background:${P.prosthesisBg};border-color:${C.blue}44` : ''}`)}>
        ...
        ${summary.right_implant ? `<div ${st(`font-size:10px;color:${C.blueLt};margin-bottom:8px;line-height:1.5;border-left:3px solid ${C.blue};padding-left:8px`)}>Hip prosthesis — BMD not applicable. Values below reflect prosthesis artifact and are excluded from bone density classification.</div>` : ''}
        ${bmdTable(rfRows, P, summary.right_implant)}
      </div>
    </div>` : ''}
