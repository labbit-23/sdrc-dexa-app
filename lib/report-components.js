...
// ── Fat-Loss Targets ──────────────────────────────────────────────────────
/**
 * Fat-loss weight goals to reach target BF%.
 * @returns {{currentWeight, leanKg, fatPct, targets}} | null if BF% is already healthy
 */
export function fatLossTargets(comp, calc, gender) {
  ...
  const currentWeight = (comp.fat_g + comp.lean_g + comp.bmc_g) / 1000

  const targets = [30, 25, 20]
    .map(pct => {
      const targetKg = parseFloat((leanKg / (1 - pct / 100)).toFixed(1))
      const lossKg = parseFloat((currentWeight - targetKg).toFixed(1))
      return { pct, targetKg, lossKg: Math.max(0, lossKg) }
    })

  return { currentWeight, leanKg, fatPct: comp.fat_pct, targets }
}
