...
function bodyFatRefCard(fatPct, gender, P) {
  ...
  // Build scale segments with proper gaps (0-maxScale range)
  const segments = []
  let pos = 0
  for (const z of zones) {
    const gapWidth = Math.max(0, z.lo - pos)
    if (gapWidth > 0) {
      segments.push({ type: 'gap', w: (gapWidth / maxScale) * 100 })
    }
    const zoneWidth = (z.hi - z.lo + 1) / maxScale * 100
    segments.push({ type: 'zone', z, w: zoneWidth })
    pos = z.hi + 1
  }
  const trailingGap = maxScale - pos
  if (trailingGap > 0) {
    segments.push({ type: 'gap', w: (trailingGap / maxScale) * 100 })
  }
  ...
  <div class="card" ${s('flex-shrink:0')}>
    ...
    <div ${s('position:relative;margin-bottom:12px')}>
      ...
      <div ${s('position:absolute;left:0;right:0;top:20px;font-size:8px;font-weight:600;display:flex')}>
        ${zones.map((z, i) => {
          const zoneStart = (z.lo / maxScale) * 100
          const zoneWidth = ((z.hi - z.lo + 1) / maxScale) * 100
