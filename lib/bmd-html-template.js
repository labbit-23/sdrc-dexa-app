...
function tbBmdRefChart(age, bmd, gender, P) {
  ...
  function zoneLabel(bmdTop, bmdBot, text, color) {
    const yTop = Math.max(yp(Math.min(bmdTop, BMD_MAX)), mt)
    const yBot = Math.min(yp(Math.max(bmdBot, BMD_MIN)), mt + cH)
    if (yBot - yTop < 6) return ''
    const yMid = ((yTop + yBot) / 2 + 2.5).toFixed(1)
    return `<text x="${(ml + cW - 3).toFixed(1)}" y="${yMid}" text-anchor="end" font-size="5.5" fill="${color}" font-weight="600" opacity="0.75">${text}</text>`
  }

  // GE Lunar iDXA total body population mean by age — sex-specific normative
  ...
  <div ${s('flex-shrink:0')}>
    ...
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${totalW} ${H}" width="${totalW}" height="${H}" style="display:block">
      ...
      ${band(BMD_MAX, t1,   '#4CAF5022')}
      ${band(t1,      t25,  '#FF980028')}
      ${band(t25,  BMD_MIN, '#F4433622')}
      ${zoneLabel(BMD_MAX, t1,   'Normal',       '#2E7D32')}
      ${zoneLabel(t1,      t25,  'Osteopenia',   '#B76E00')}
      ${zoneLabel(t25,  BMD_MIN, 'Osteoporosis', '#B71C1C')}
      ...
      <polyline points="${refPts}" fill="none" stroke="#1565C0" stroke-width="1.5" opacity="0.75"/>
      ...
      ${lineLabel(t1,  'T = −1',   '#B76E00')}
      ${lineLabel(t2,  'T = −2',   '#C62828')}
    ...
    <div ${s('display:flex;flex-wrap:wrap;gap:4px 10px;font-size:7px;margin-top:4px;color:#666')}>
      <span>● Patient BMD</span>
      <span style="display:inline-flex;align-items:center;gap:3px"><svg width="14" height="5" style="display:block"><line x1="0" y1="2.5" x2="14" y2="2.5" stroke="#1565C0" stroke-width="1.5" opacity="0.75"/></svg>Pop. mean (${male ? 'male' : 'female'} norm)</span>
      <span style="display:inline-flex;align-items:center;gap:3px"><svg width="8" height="8" style="display:block"><rect width="8" height="8" fill="#4CAF5055"/></svg>Normal</span>
      <span style="display:inline-flex;align-items:center;gap:3px"><svg width="8" height="8" style="display:block"><rect width="8" height="8" fill="#FF980055"/></svg>Osteopenia</span>
      <span style="display:inline-flex;align-items:center;gap:3px"><svg width="8" height="8" style="display:block"><rect width="8" height="8" fill="#F4433655"/></svg>Osteoporosis</span>
    </div>
  </div>`
}
