...
function bodyFatRefCard(fatPct, gender, P) {
  ...
  const zones = male
    ? [{ lbl: 'Athletic', range: '6–13%',   w: 12, c: '#1F77B4', lo: 6,  hi: 12.9 },   // blue
       ...
       { lbl: 'Overweight', range: '24–29%', w: 7.5, c: C.amber, lo: 24, hi: 29 },   // orange
    ...
    : [{ lbl: 'Athletic', range: '10–20%',  w: 16, c: '#1F77B4', lo: 10,  hi: 19.9 },   // blue
       ...
       { lbl: 'Overweight', range: '31–36%', w: 8.2, c: C.amber, lo: 31, hi: 36 },   // orange
  ...
  <div class="card" ${s('flex-shrink:0')}>
    ...
    <div ${s('position:relative;height:14px;margin-top:16px')}>
      ${(() => {
        const ticks = []
        // Show % values at each zone boundary in grey
        zones.forEach((z, i) => {
          const pos = ((z.lo - firstZoneLo) / scaleRange) * 100
          ticks.push(`<div ${s(`position:absolute;left:${pos}%;transform:translateX(-50%);font-size:8px;color:${P.gray};font-weight:600`)}>${z.lo}%</div>`)
        })
        // End marker
...
function page5(data, P, lh = false, pageNum = 5, totalPages = 5) {
  ...
  const fatTargetsBlock = (() => {
    ...
    <div class="card" ${s('flex-shrink:0')}>
      ...
      <div ${s(`font-size:9px;color:${targets.almiStatus === 'low' ? C.amber : P.text};padding:6px 8px;background:${targets.almiStatus === 'low' ? P.statusWarn : P.statusInfo};border-left:3px solid ${targets.almiStatus === 'low' ? C.amber : C.tealLt};margin-bottom:8px;line-height:1.5`)}>${targets.almiGuidance} ${targets.preservationNote}</div>
