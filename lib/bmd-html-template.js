...
function tbBmdRefChart(age, bmd, gender, P) {
  ...
  const peak = male ? 1.285 : 1.220
  const sd   = male ? 0.110 : 0.080

  // T-score thresholds (young-adult, fixed horizontal)
  const tNorm = peak          // T = 0
  ...
  // Age-matched (Z-score) curves: population mean ± k SD
  function curvePts(kSD) {
    return tbRef.map(([a, b]) => [xp(a), yp(b - kSD * sd)])
  }
  const c0  = curvePts(0)    // population mean
  const c1  = curvePts(1)    // Z-score Normal/Osteopenia boundary
  const c25 = curvePts(2.5)  // Z-score Osteopenia/Osteoporosis boundary

  const pts = arr => arr.map(([x, y]) => `${x.toFixed(1)},${cy(y).toFixed(1)}`).join(' ')

  // Z-score curved zone polygons (slightly darker overlay)
  ...
  // T-score horizontal band helper
  ...
  // T-score horizontal line + label helper
  ...
  // Zone label positions at age ~60 midpoint of each Z-score band
  ...
  <div ${s('flex-shrink:0')}>
    ...
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${totalW} ${H}" width="${totalW}" height="${H}" style="display:block">
      ...
      <polygon points="${polyNormal}"  fill="#4CAF5020" clip-path="url(#bmd-clip)"/>
      <polygon points="${polyOstopen}" fill="#FF98002a" clip-path="url(#bmd-clip)"/>
      <polygon points="${polyOsteo}"   fill="#F443361e" clip-path="url(#bmd-clip)"/>
      ...
      ${tLine(tOst,  'T=−1')}
      ${tLine(tOstP, 'T=−2.5')}
      <polyline points="${pts(c1)}"  fill="none" stroke="#B76E00" stroke-width="1"   stroke-dasharray="4,2" opacity="0.8" clip-path="url(#bmd-clip)"/>
      <polyline points="${pts(c25)}" fill="none" stroke="#B71C1C" stroke-width="1"   stroke-dasharray="4,2" opacity="0.8" clip-path="url(#bmd-clip)"/>
    ...
    <div ${s('display:flex;align-items:center;flex-wrap:nowrap;gap:5px;font-size:6.5px;margin-top:3px;color:#666;overflow:hidden')}>
      <span style="white-space:nowrap">● Patient</span>
      <span style="display:inline-flex;align-items:center;gap:2px;white-space:nowrap"><svg width="12" height="4" style="display:block"><line x1="0" y1="2" x2="12" y2="2" stroke="#1565C0" stroke-width="1.5" opacity="0.75"/></svg>Pop. mean</span>
