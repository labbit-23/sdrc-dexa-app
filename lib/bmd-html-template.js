...
const _bmNorm = (() => {
  const csv   = readFileSync(join(process.cwd(), 'lib/bmd-norm.csv'), 'utf8')
...
function tbBmdRefChart(age, bmd, gender, P) {
  ...
  const tbNorm = _bmNorm.totalbody
  const { peak, sd } = male ? tbNorm.male : tbNorm.female
  const tbRef = (male ? tbNorm.male : tbNorm.female).curve
