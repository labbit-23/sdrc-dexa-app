...
function bodyFatRefCard(fatPct, gender, P) {
  ...
  // ACE Categories with corrected ranges and proportional widths
  const zones = male
    ? [{ lbl: 'Athletic', range: '6–13%',   w: 28,   c: C.greenLt, lo: 6,  hi: 13 },
       { lbl: 'Fit',      range: '14–17%',  w: 12,   c: C.tealLt,  lo: 14, hi: 17 },
       { lbl: 'Normal',   range: '18–24%',  w: 21,   c: C.amber,   lo: 18, hi: 24 },
       { lbl: 'Excess',   range: '25%+',    w: 39,   c: C.red,     lo: 25, hi: 100 }]
    : [{ lbl: 'Athletic', range: '14–20%',  w: 17.5, c: C.greenLt, lo: 14, hi: 20 },
       { lbl: 'Fit',      range: '21–24%',  w: 8.3,  c: C.tealLt,  lo: 21, hi: 24 },
       { lbl: 'Normal',   range: '25–31%',  w: 19.2, c: C.amber,   lo: 25, hi: 31 },
       { lbl: 'Excess',   range: '32%+',    w: 55,   c: C.red,     lo: 32, hi: 100 }]
  ...
  <div class="card" ${s('flex-shrink:0')}>
    ...
    <div ${s('position:relative;margin-bottom:12px')}>
      ...
      <div ${s('display:flex;margin-top:4px;font-size:8px;font-weight:600')}>
        ${zones.map(z => `<div ${s(`width:${z.w}%;text-align:center;color:${z.c}`)}>${z.lbl}</div>`).join('')}
      </div>
    </div>
    <div ${s('display:flex;justify-content:space-between;font-size:9px;margin-top:2px')}>
      ${zones.map(z => `<div ${s(`flex:1;text-align:center;color:${P.gray}`)}>${z.range}</div>`).join('')}
    </div>
