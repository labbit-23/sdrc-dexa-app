...
function page5(data, P, lh = false, pageNum = 5, totalPages = 5) {
  ...
  const fatTargetsBlock = (() => {
    ...
    if (!targets) return ''
    const rows = targets.targets.map(({ pct, fatToLose, targetWeight }) => {
      return `<tr>
        <td ${s(`padding:5px 10px;font-size:11px;color:${P.gray}`)}>${pct}% body fat</td>
        <td ${s(`padding:5px 10px;font-size:11px;font-weight:700;color:${P.text};font-family:monospace;text-align:right`)}>${fatToLose} kg</td>
        <td ${s(`padding:5px 10px;font-size:11px;font-weight:700;color:${P.text};font-family:monospace;text-align:right`)}>${targetWeight} kg</td>
      </tr>`
    }).join('')
    ...
    <div class="card" ${s('flex-shrink:0')}>
      ...
      <div ${s(`font-size:9px;color:${P.gray};margin-bottom:8px`)}>
        Current: <strong ${s(`color:${P.text}`)}>${targets.fatPct}% body fat · ${targets.currentWeight} kg total</strong> — Assumes lean mass preserved at ${targets.leanKg} kg
      </div>
      <table ${s('width:100%;border-collapse:collapse')}>
        <thead><tr>
          ...
          <th ${s(`padding:4px 10px;font-size:9px;font-weight:700;color:${P.gray};text-align:right;border-bottom:1px solid ${P.border}`)}>Target Weight</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
      <div ${s(`font-size:8.5px;color:${P.gray};margin-top:8px;line-height:1.5`)}>${targets.preservationNote}</div>
      ${sarcoWarning}
    </div>`
  })()
