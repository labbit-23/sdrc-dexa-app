...
function tbBmdRefChart(age, bmd, gender, P) {
  if (age == null || bmd == null || bmd === 0) return ''

  const male = (gender || '').toLowerCase().startsWith('m')

  const W = 220, H = 110
  const ml = 26, mr = 3, mt = 5, mb = 18
  ...
  // GE Lunar iDXA total body normative values by sex
  // Female: peak ≈ 1.220 g/cm², SD ≈ 0.080  |  Male: peak ≈ 1.285 g/cm², SD ≈ 0.110
  const sd = male ? 0.110 : 0.080

  const tbRefFemale = [[20,1.195],[30,1.220],[40,1.190],[50,1.155],[55,1.130],[60,1.095],[65,1.058],[70,1.020],[75,0.982],[80,0.945],[90,0.900]]
  const tbRefMale   = [[20,1.254],[30,1.285],[40,1.267],[50,1.245],[55,1.230],[60,1.215],[65,1.195],[70,1.175],[75,1.152],[80,1.130],[90,1.080]]
  const tbRef = male ? tbRefMale : tbRefFemale

  // Three curves: population mean, mean−1 SD (Normal/Osteopenia boundary),
  // mean−2.5 SD (Osteopenia/Osteoporosis boundary).
  // All three follow the same shape — zones flow with the age-matched mean.
  function curvePts(kSD) {
    return tbRef.map(([a, b]) => [xp(a), yp(b - kSD * sd)])
  }
  const c0   = curvePts(0)    // population mean
  const c1   = curvePts(1)    // Normal / Osteopenia boundary
  const c25  = curvePts(2.5)  // Osteopenia / Osteoporosis boundary

  const pts = arr => arr.map(([x, y]) => `${x.toFixed(1)},${cy(y).toFixed(1)}`).join(' ')

  // Zone polygons: each band is the area between two curves (or chart edge)
  ...
  // Zone label positions: midpoint of each band at age ~60 (stable, mid-chart)
  const iMid = tbRef.findIndex(([a]) => a >= 60)
  const ym1   = cy(c1[iMid][1])
  const ym25  = cy(c25[iMid][1])
  const xLbl  = (c1[iMid][0] + 18).toFixed(1)  // slightly right of age 60
  ...
  <div ${s('flex-shrink:0')}>
    ...
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${totalW} ${H}" width="${totalW}" height="${H}" style="display:block">
      <defs><clipPath id="bmd-clip"><rect x="${ml}" y="${mt}" width="${cW}" height="${cH}"/></clipPath></defs>
      <rect x="${ml}" y="${mt}" width="${cW}" height="${cH}" fill="#f9fafb"/>
      <polygon points="${polyNormal}"  fill="#4CAF5022" clip-path="url(#bmd-clip)"/>
      <polygon points="${polyOstopen}" fill="#FF980030" clip-path="url(#bmd-clip)"/>
      <polygon points="${polyOsteo}"   fill="#F4433625" clip-path="url(#bmd-clip)"/>
      ${xTicks.map(a => `<line x1="${xp(a).toFixed(1)}" y1="${mt}" x2="${xp(a).toFixed(1)}" y2="${mt+cH}" stroke="#ddd" stroke-width="0.5"/>`).join('')}
      ${yTicks.map(([bv]) => `<line x1="${ml}" y1="${yp(bv).toFixed(1)}" x2="${ml+cW}" y2="${yp(bv).toFixed(1)}" stroke="#ddd" stroke-width="0.5"/>`).join('')}
      <polyline points="${pts(c1)}"  fill="none" stroke="#B76E00" stroke-width="1"   stroke-dasharray="4,2" opacity="0.85" clip-path="url(#bmd-clip)"/>
      <polyline points="${pts(c25)}" fill="none" stroke="#B71C1C" stroke-width="1"   stroke-dasharray="4,2" opacity="0.85" clip-path="url(#bmd-clip)"/>
      <polyline points="${pts(c0)}"  fill="none" stroke="#1565C0" stroke-width="1.5"                        opacity="0.75" clip-path="url(#bmd-clip)"/>
    ...
    <div ${s(`display:flex;flex-wrap:wrap;gap:3px 8px;font-size:7px;margin-top:4px;color:#666;max-width:${totalW}px`)}>
      <span>● Patient BMD</span>
      <span style="display:inline-flex;align-items:center;gap:3px"><svg width="14" height="5" style="display:block"><line x1="0" y1="2.5" x2="14" y2="2.5" stroke="#1565C0" stroke-width="1.5" opacity="0.75"/></svg>Pop. mean (${male ? 'male' : 'female'})</span>
      <span style="display:inline-flex;align-items:center;gap:2px"><svg width="7" height="7" style="display:block"><rect width="7" height="7" fill="#4CAF5055"/></svg>Normal</span>
      <span style="display:inline-flex;align-items:center;gap:2px"><svg width="7" height="7" style="display:block"><rect width="7" height="7" fill="#FF980055"/></svg>Osteopenia</span>
      <span style="display:inline-flex;align-items:center;gap:2px"><svg width="7" height="7" style="display:block"><rect width="7" height="7" fill="#F4433655"/></svg>Osteoporosis</span>
    </div>
  </div>`
}
...
function page2(data, P, lh = false, pageNum = 2, totalPages = 5) {
  ...
  <div class="card" ${s('flex-shrink:0')}>
    ...
    <div ${s('display:flex;align-items:center;gap:12px')}>
      ...
      <div ${s('flex:1')}>
        <div ${s(`font-size:10px;color:${P.text};font-weight:600`)}>Body fat higher than ${c}% of same-age, same-sex peers</div>
  ...
  <div class="col" ${s('flex:1;gap:14px;min-height:0')}>
    ...
    <div class="card" ${s(`background:${boneCardBg(bone.classification, P)};flex-shrink:0`)}>
      <div class="row" ${s('align-items:stretch;gap:24px;margin-bottom:12px')}>
        ...
        ${tbBmdRefChart(pt.age, bone.total_bmd, pt.gender, P)}
      </div>
...
<div class="page">
  ...
  <div class="card">
    ...
    <div class="row" ${s('gap:14px;align-items:flex-start')}>
      ...
      <div ${s('flex:1')}>
        ...
        <div ${s(`font-size:8px;color:${P.gray};margin-top:4px;line-height:1.5`)}>Multiply RMR by activity level to estimate daily calorie needs for weight maintenance.</div>
      </div>
    </div>
  </div>
  ...
  <div ${s(`border-top:1px solid ${P.border};padding-top:7px;font-size:9px;color:${P.gray};line-height:1.7`)}>
    <div>
