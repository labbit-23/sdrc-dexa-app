...
function page5(data, P, lh = false, pageNum = 5, totalPages = 5) {
  ...
  const fatTargetsBlock = (() => {
    ...
    <div class="card" ${s('flex-shrink:0')}>
      ...
      <div ${s(`font-size:9px;color:${P.gray};margin-bottom:8px`)}>
        Current: <strong ${s(`color:${P.text}`)}>${targets.fatPct}% body fat · ${targets.currentWeight.toFixed(1)} kg total</strong> · Preserving lean mass at ${targets.leanKg.toFixed(1)} kg
      </div>
