...
function bodyFatRefCard(fatPct, gender, P) {
  ...
  const zones = male
    ? [{ lbl: 'Athletic', range: '6–13%',   w: 12, c: '#1F77B4', lo: 6,  hi: 12.9 },   // blue
       { lbl: 'Fit',      range: '13–17%',  w: 4.7, c: '#2CA02C', lo: 13, hi: 17 },   // green
  ...
  <div class="card" ${s('flex-shrink:0')}>
    ...
    <div ${s('position:relative;height:14px;margin-top:16px')}>
      ${(() => {
        const ranges = []
        // Show range under each category in grey
        zones.forEach((z, i) => {
          const zoneStart = (z.lo - firstZoneLo) / scaleRange * 100
          const zoneWidth = (z.hi - z.lo + 1) / scaleRange * 100
          const rangeText = z.range.replace('–', '-')
          ranges.push(`<div ${s(`position:absolute;left:${zoneStart}%;width:${zoneWidth}%;text-align:center;font-size:8px;color:${P.gray};font-weight:600`)}>${rangeText}</div>`)
        })
        return ranges.join('')
      })()}
    </div>
