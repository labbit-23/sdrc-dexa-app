...
/* Letterhead: same design as light theme, logo hidden, 45mm bottom margin for pre-printed footer. */
const letterheadCss = `
  @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap');
  *{box-sizing:border-box;margin:0;padding:0}
  body{background:${lightPal.bg};font-family:'Inter',sans-serif;color:${lightPal.text};
       -webkit-print-color-adjust:exact;print-color-adjust:exact}
  .page{width:210mm;height:252mm;padding:10mm 13mm;margin:0 auto;
        background:${lightPal.bg};page-break-after:always;position:relative;
        display:flex;flex-direction:column;overflow:hidden}
  .page:last-child{page-break-after:auto}
  @page{size:A4;margin:0 0 45mm 0}
  .row{display:flex;gap:14px}
  .col{display:flex;flex-direction:column}
  .card{background:${lightPal.card};border:1px solid ${lightPal.border};border-radius:8px;padding:16px}
  .lbl{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.8px;color:${lightPal.gray}}
  .sec{font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:${C.teal};margin-bottom:10px}
  .tag{display:inline-block;padding:4px 12px;border-radius:3px;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px}
...
function pageTrends(data, history, P, lh, pageNum, total) {
  ...
  // ── Footer ───────────────────────────────────────────────────────────────
  const foot = `
  <div class="page-footer" style="flex-shrink:0;margin-top:10px;border-top:1px solid ${P.border};padding-top:7px;font-size:9px;color:${P.gray};line-height:1.7">
    Scanner ${esc(pt.scanner)} · ${esc(pt.software)} · Showing up to 5 most recent scans ·
    Δ arrows: <span style="color:${C.greenLt}">green = beneficial change</span> · <span style="color:${C.red}">red = adverse change</span> · For clinical use only.
  </div>`
...
<style>
${css}
body{background:#6b7280!important;padding:20px;display:flex;flex-wrap:wrap;gap:16px;justify-content:center;align-items:flex-start}
.page{height:297mm!important;transform:scale(0.5);transform-origin:top left;margin-right:calc(-210mm * 0.5);margin-bottom:calc(-297mm * 0.5 + 16px);box-shadow:0 4px 20px rgba(0,0,0,.4);flex-shrink:0}
@media print{
  body{background:white!important;display:block;padding:0;gap:0}
  .page{height:${letterhead ? '252mm' : '297mm'}!important;overflow:hidden!important;transform:none!important;margin:0!important;box-shadow:none!important;page-break-after:always;display:flex}
  .page:last-child{page-break-after:auto}
}
</style></head><body>
${pagesHtml}
...
function header(pt, title, P, lh = false, pageNum = 0, totalPages = 0) {
  ...
  const logoRow = lh ? '' : `
    <div class="page-logo-row" ${s('display:flex;justify-content:space-between;align-items:center;padding-bottom:8px;margin-bottom:8px;border-bottom:1px solid ' + P.border)}>
      <img src="https://www.sdrc.in/assets/sdrc-logo-full.png" alt="SDRC Diagnostics" ${s(`height:36px;width:auto;border-radius:4px;padding:${P === darkPal ? '2px 4px' : '0'};background:${P === darkPal ? 'rgba(255,255,255,0.92)' : 'transparent'}`)}>
  ...
  const patientCompact = lh ? `
    <div ${s(`display:flex;justify-content:space-between;align-items:baseline;margin-bottom:2px`)}>
      <div ${s(`font-size:20px;font-weight:800;color:${P.text};letter-spacing:.3px`)}>${esc(title)}</div>
      ${pgTag}
    </div>
    <div ${s(`font-size:9.5px;color:${P.gray};margin-bottom:4px`)}>
      <span ${s(`font-weight:700;color:${P.text}`)}>${esc(pt.name)}</span>
      &nbsp;·&nbsp; ID: ${esc(pt.id)} &nbsp;·&nbsp; ${esc(pt.gender)} · ${pt.age}y · ${pt.height_cm} cm · ${pt.weight_entered_kg} kg · Scan ${esc(pt.scan_date)}
    </div>` : `
...
function symmetryCard(bilateral, P) {
  if (!bilateral) return ''
  const { arms, legs } = bilateral
  if (!arms) return ''

  // Clinical asymmetry thresholds differ by limb
  // Arms: dominant side naturally 5-10% larger; >15% notable, >25% significant
  // Legs: should be near-symmetric; >10% notable, >15% significant
  function asymFlag(pct, isArm) {
    const hi = isArm ? 25 : 15, lo = isArm ? 15 : 10
    if (pct >= hi) return { color: C.red,   label: 'Significant' }
    if (pct >= lo) return { color: C.amber, label: 'Notable' }
    return           { color: C.greenLt, label: 'Symmetric' }
  }

  function splitBar(left_g, right_g, color) {
    const total = left_g + right_g
    if (total === 0) return ''
    const lp = (left_g / total * 100).toFixed(1)
    const rp = (right_g / total * 100).toFixed(1)
    const lkg = (left_g / 1000).toFixed(2)
    const rkg = (right_g / 1000).toFixed(2)
    return `
      <div ${s('display:flex;align-items:center;gap:6px;margin-bottom:4px')}>
        <div ${s(`font-size:8.5px;color:${P.gray};width:28px;text-align:right;flex-shrink:0`)}>
          L<br>${lp}%
        </div>
        <div ${s('flex:1;height:18px;display:flex;border-radius:3px;overflow:hidden;background:#e5e7eb')}>
          <div ${s(`width:${lp}%;background:${color};opacity:0.85;display:flex;align-items:center;justify-content:flex-end;padding-right:3px`)}>
            ${parseFloat(lp) > 25 ? `<span ${s('font-size:8px;color:#fff;font-weight:700')}>${lkg}</span>` : ''}
          </div>
          <div ${s(`width:${rp}%;background:${color};opacity:0.6;display:flex;align-items:center;padding-left:3px`)}>
            ${parseFloat(rp) > 25 ? `<span ${s('font-size:8px;color:#fff;font-weight:700')}>${rkg}</span>` : ''}
          </div>
        </div>
        <div ${s(`font-size:8.5px;color:${P.gray};width:28px;flex-shrink:0`)}>
          R<br>${rp}%
        </div>
      </div>`
  }

  function limbRow(label, side, isArm) {
    if (!side) return ''
    const flag = asymFlag(side.lean_asym, isArm)
    const dom  = side.dominant === 'left' ? 'L dominant' : 'R dominant'
    return `
      <div ${s('margin-bottom:10px')}>
        <div ${s('display:flex;align-items:baseline;justify-content:space-between;margin-bottom:3px')}>
          <span ${s('font-size:10px;font-weight:700;color:' + P.text)}>${label}</span>
          <span ${s(`font-size:9px;font-weight:700;color:${flag.color}`)}>
            ${side.lean_asym}% Δ · ${flag.label}
          </span>
        </div>
        <div ${s(`font-size:9px;color:${P.gray};margin-bottom:3px`)}>
          Lean mass · ${dom}
          &nbsp;·&nbsp;
          L: ${(side.left.lean_g/1000).toFixed(2)} kg &nbsp; R: ${(side.right.lean_g/1000).toFixed(2)} kg
        </div>
        ${splitBar(side.left.lean_g, side.right.lean_g, C.teal)}
        <div ${s(`font-size:9px;color:${P.gray};margin-bottom:3px;margin-top:2px`)}>
          Fat mass · L: ${(side.left.fat_g/1000).toFixed(2)} kg &nbsp; R: ${(side.right.fat_g/1000).toFixed(2)} kg
        </div>
        ${splitBar(side.left.fat_g, side.right.fat_g, C.pink)}
      </div>`
  }

  return `
  <div class="card" ${s('flex-shrink:0')}>
    <div class="sec">Bilateral Symmetry</div>
    <div ${s(`font-size:9px;color:${P.gray};margin-bottom:8px`)}>
      Left vs Right distribution — complements A/G (horizontal) with vertical symmetry analysis.
      Lean asymmetry reflects muscle dominance; fat asymmetry may indicate postural or activity patterns.
    </div>
    ${limbRow('Arms', arms, true)}
    ${limbRow('Legs', legs, false)}
  </div>`
}
...
function page1(data, P, lh = false, pageNum = 1, totalPages = 5) {
  ...
  return `
<div class="page">
  ${header(pt, 'DEXA — Body Composition Summary', P, lh, pageNum, totalPages)}
  <div ${s('flex:1;display:flex;gap:16px;min-height:0')}>
    ...
    <div class="col" ${s('flex:1;gap:12px;min-height:0')}>
      ...
      <div class="card" ${s('flex-shrink:0')}>
        <div class="sec">Resting Metabolic Rate (RMR)</div>
        <div class="row" ${s('gap:14px;align-items:flex-start')}>
          <div ${s('flex:1')}>
            <div ${s('display:flex;align-items:baseline;gap:4px;margin-bottom:5px')}>
              ...
              <span ${s(`font-size:10px;color:${P.gray}`)}>kcal/day</span>
            </div>
            <div ${s(`font-size:9px;color:${P.gray};line-height:1.65`)}>
              Calories burned <strong ${s(`color:${P.text}`)}>at complete rest</strong> — organ function, circulation, thermoregulation.
              Derived from lean mass via Katch-McArdle, which is more accurate than weight-based formulas.
              <strong ${s(`color:${P.text}`)}>Higher lean mass raises RMR</strong>, making weight management easier long-term.
            </div>
          </div>
          <div ${s(`width:1px;background:${P.border};align-self:stretch`)}></div>
          <div ${s('flex:1')}>
            <div ${s(`font-size:8px;font-weight:700;color:${P.gray};text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px`)}>Total Daily Energy Needs</div>
            <div ${s('display:grid;grid-template-columns:1fr 1fr;gap:2px 10px')}>
              ${[
                ...
                return `<div ${s('display:flex;justify-content:space-between;align-items:baseline;padding:2px 0;border-bottom:1px solid ' + P.border)}>
                  ...
                  <span ${s(`font-size:9.5px;font-weight:700;color:${C.tealLt};font-family:monospace`)}>${kcal.toLocaleString()}</span>
                </div>`
              }).join('')}
            </div>
            <div ${s(`font-size:8px;color:${P.gray};margin-top:4px;line-height:1.5`)}>Multiply RMR by your activity level to estimate daily calorie needs for weight maintenance.</div>
          </div>
        </div>
      </div>
    </div>
...
function page2(data, P, lh = false, pageNum = 2, totalPages = 5) {
  const { patient: pt, composition: comp, bilateral } = data
  ...
  return `
<div class="page">
  ${header(pt, 'DEXA — Fat Distribution Analysis', P, lh, pageNum, totalPages)}
  <div ${s('flex:1;display:flex;gap:16px;min-height:0')}>
    <div ${s('width:160px;flex-shrink:0;display:flex;flex-direction:column')}>
...
function page3(data, P, lh = false, pageNum = 3, totalPages = 5) {
  ...
  return `
<div class="page">
  ${header(pt, 'DEXA — Regional Body Composition', P, lh, pageNum, totalPages)}
...
function page4(data, P, lh = false, pageNum = 4, totalPages = 5) {
  ...
  return `
<div class="page">
  ${header(pt, 'DEXA — Bone Health & Density', P, lh, pageNum, totalPages)}
  <div class="col" ${s('flex:1;gap:14px;min-height:0')}>
    ...
    <div class="row" ${s('flex:1;gap:14px;min-height:0')}>
      <div ${s('width:130px;flex-shrink:0;display:flex;flex-direction:column;gap:4px')}>
        ...
        <div class="card" ${s('flex-shrink:0;padding:10px 14px')}>
          <div class="row" ${s('gap:20px;align-items:flex-start')}>
            ...
            <div ${s('flex:1')}>
              ...
              <div ${s(`margin-top:4px;font-size:9px;color:${P.gray}`)}>If T is low but Z is near 0, bone loss is age-related, not accelerated.</div>
            </div>
          </div>
        </div>
...
function page5(data, P, lh = false, pageNum = 5, totalPages = 5) {
  ...
  return `
<div class="page">
  ${header(pt, 'DEXA — Clinical Summary', P, lh, pageNum, totalPages)}
  <div ${s('flex:1;display:grid;grid-template-columns:1fr 1fr;grid-auto-rows:auto;align-content:start;gap:12px;min-height:0')}>
    ${cards}
  </div>
  <div ${s(`flex-shrink:0;margin-top:10px;background:${P.card};border:1px solid ${P.border};border-left:4px solid ${C.tealLt};border-radius:6px;padding:10px 16px`)}>
