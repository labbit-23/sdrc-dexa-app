...
function tbSparkline(values, W, H, color) {
  ...
  const pctChanges = pts.map(v => firstVal !== 0 ? ((v - firstVal) / Math.abs(firstVal)) * 100 : 0)
  const min = Math.min(...pctChanges), max = Math.max(...pctChanges)
  const range = Math.max(Math.abs(min), Math.abs(max)) * 1.2 || 0.001
  const idxs = values.map((v, i) => ({ v, i, pctChange: pctChanges[values.filter(x => x != null).indexOf(v)] })).filter(x => x.v != null)
  const xs = idxs.map(x => ((x.i / (values.length - 1)) * (W - 12) + 6).toFixed(1))
  const ys = idxs.map(x => (H / 2 - (x.pctChange / range) * (H - 20)).toFixed(1))
  const pts2 = xs.map((x, i) => `${x},${ys[i]}`).join(' ')
  const dots = xs.map((x, i) => `<circle cx="${x}" cy="${ys[i]}" r="2.5" fill="${color}"/>`).join('')
  const labels = idxs.map((x, i) => {
    let label = x.v
    if (typeof x.v === 'number') {
      if (Number.isInteger(x.v)) {
        label = x.v.toString()
      } else {
        label = x.v.toFixed(1)
      }
    }
    const textY = parseFloat(ys[i]) - 7
    return `<text x="${xs[i]}" y="${textY}" text-anchor="middle" font-size="6.5" fill="${color}" font-weight="500">${label}</text>`
  }).join('')
  return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H-4}" style="display:block;overflow:visible">
    <line x1="6" y1="${H-6}" x2="${W-6}" y2="${H-6}" stroke="#e0e0e0" stroke-width="0.5"/>
    <line x1="6" y1="0" x2="6" y2="${H-6}" stroke="#e0e0e0" stroke-width="0.5"/>
...
function tbSparklineDual(values1, values2, W, H, color1, color2, label1, label2) {
  const pts1 = values1.filter(v => v != null)
  const pts2 = values2.filter(v => v != null)
  if (pts1.length < 2 || pts2.length < 2) return ''

  const first1 = pts1[0], first2 = pts2[0]
  const pct1 = values1.map(v => first1 !== 0 ? ((v - first1) / Math.abs(first1)) * 100 : 0)
  const pct2 = values2.map(v => first2 !== 0 ? ((v - first2) / Math.abs(first2)) * 100 : 0)

  const allPct = [...pct1.filter(v => v !== 0), ...pct2.filter(v => v !== 0)]
  const min = Math.min(...allPct), max = Math.max(...allPct)
  const range = Math.max(Math.abs(min), Math.abs(max)) * 1.2 || 0.001

  const buildLine = (values, pctChanges, color) => {
    const idxs = values.map((v, i) => ({ v, i, pctChange: pctChanges[i] })).filter(x => x.v != null)
    const xs = idxs.map(x => ((x.i / (values.length - 1)) * (W - 12) + 6).toFixed(1))
    const ys = idxs.map(x => (H / 2 - (x.pctChange / range) * (H - 20)).toFixed(1))
    const pts = xs.map((x, i) => `${x},${ys[i]}`).join(' ')
    const dots = xs.map((x, i) => `<circle cx="${x}" cy="${ys[i]}" r="2.5" fill="${color}"/>`).join('')
    return { pts, dots, xs, ys, idxs }
  }

  const line1 = buildLine(values1, pct1, color1)
  const line2 = buildLine(values2, pct2, color2)

  return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H-4}" style="display:block;overflow:visible">
    <line x1="6" y1="${H-6}" x2="${W-6}" y2="${H-6}" stroke="#e0e0e0" stroke-width="0.5"/>
    <line x1="6" y1="0" x2="6" y2="${H-6}" stroke="#e0e0e0" stroke-width="0.5"/>
    <polyline points="${line1.pts}" fill="none" stroke="${color1}" stroke-width="1.5" stroke-linejoin="round"/>
    <polyline points="${line2.pts}" fill="none" stroke="${color2}" stroke-width="1.5" stroke-linejoin="round"/>
    ${line1.dots}
    ${line2.dots}
  </svg>`
}
...
function pageTrends(data, history, P, lh, pageNum, total) {
  ...
  const dateHeaders = scans.map((sc, i) => {
    ...
    return `<th style="padding:5px 8px;font-size:9px;font-weight:${isCurrent ? 700 : 500};color:${isCurrent ? C.tealLt : P.gray};text-align:right;border-bottom:2px solid ${P.border}">${tbFmtDate(sc.patient?.scan_date)}${isCurrent ? ' ★' : ''}</th>`
  }).join('')

  const metricRows = metricDefs.map(({ label, get, unit, good }) => {
    const vals = scans.map(sc => get(sc))
    if (vals.every(v => v == null)) return ''
    const showDeltaAsPercent = label === 'Fat Mass' || label === 'Lean Mass'
    const cells = vals.map((v, i) => {
      ...
      if (v != null && prev != null) {
        ...
        if (showDeltaAsPercent && prev !== 0) {
          delta = ((v - prev) / prev) * 100
          displayDelta = `${delta > 0 ? '+' : ''}${delta.toFixed(1)}%`
        } else {
          delta = v - prev
          displayDelta = `${delta > 0 ? '+' : ''}${Math.abs(delta) < 1 ? delta.toFixed(2) : delta.toFixed(1)}`
        }
        const beneficial = good === 'down' ? delta < 0 : delta > 0
        const color = beneficial ? C.greenLt : delta === 0 ? P.gray : C.red
        const arrow = delta > 0 ? '↑' : delta < 0 ? '↓' : '→'
        deltaHtml = `<div style="font-size:8px;color:${color}">${displayDelta} ${arrow}</div>`
      }
      const display = v != null ? (typeof v === 'number' && !Number.isInteger(v) ? v : v) : '—'
      return `<td style="padding:5px 8px;text-align:right;vertical-align:top;border-bottom:1px solid ${P.border}">
        <div style="font-size:10px;color:${P.text};font-family:monospace">${display}</div>
        ${deltaHtml}
      </td>`
    }).join('')
    return `<tr>
      <td style="padding:5px 8px;font-size:10px;color:${P.grayLt};border-bottom:1px solid ${P.border};white-space:nowrap;vertical-align:top">${label}</td>
      <td style="padding:5px 8px;font-size:9px;color:${P.gray};border-bottom:1px solid ${P.border};white-space:nowrap;vertical-align:top">${unit}</td>
      ${cells}
    </tr>`
  }).filter(Boolean).join('')
  ...
  // ── Footer ───────────────────────────────────────────────────────────────
  const foot = `
  <div style="flex-shrink:0;margin-top:12px;border-top:1px solid ${P.border};padding-top:6px;font-size:8px;color:${P.gray};line-height:1.5">
    Scanner ${esc(pt.scanner)} · ${esc(pt.software)} · Showing up to 5 most recent scans · Δ arrows: <span style="color:${C.greenLt}">green = beneficial</span> · <span style="color:${C.red}">red = adverse</span> · For clinical use only.
  </div>`
      ...
      <table style="width:100%;border-collapse:collapse">
        <thead>
          <tr>
            ...
            <th style="padding:5px 8px;border-bottom:2px solid ${P.border}"></th>
            ${dateHeaders}
          </tr>
        </thead>
...
function symmetryCard(bilateral, P) {
  ...
  function row(label, side, region, isLast = false) {
    ...
    const td = (v, clr, massType) => {
      const bgMap = { lean: `background:${C.cyan}10;`, fat: `background:${C.amber}10;`, bone: `background:${P.border}10;` }
      const bg = bgMap[massType] || ''
      return `<td ${s(`padding:5px 6px;font-size:9.5px;color:${clr};text-align:right;${fw}${bg}`)}>${v}</td>`
    }
    ...
    return `<tr ${s(border)}>
      ...
      ${td(kg(side.left.lean_g),  P.text, 'lean')}
      ${td(kg(side.right.lean_g), P.text, 'lean')}
      ${td(`<span style="color:${lc};font-weight:700">${side.lean_asym}% ${leanDom}</span>`, lc, 'lean')}
      ${td(kg(side.left.fat_g),   P.text, 'fat')}
      ${td(kg(side.right.fat_g),  P.text, 'fat')}
      ${td(`<span style="color:${fc};font-weight:700">${side.fat_asym}% ${fatDom}</span>`,  fc, 'fat')}
      ${td(kg(side.left.bone_g),  P.text, 'bone')}
      ${td(kg(side.right.bone_g), P.text, 'bone')}
      ${td(`<span style="color:${bc};font-weight:700">${side.bone_asym}%</span>`, bc, 'bone')}
    </tr>`
  }
  ...
  const thCatMap = { lean: C.cyan, fat: C.amber, bone: P.border }
  const thCat = (massType) => s(`padding:6px 6px;font-size:8.5px;font-weight:700;text-align:center;border-bottom:2px solid ${P.border};background:${thCatMap[massType]}40;color:${P.text}`)

  return `
  <div class="card" ${s('flex-shrink:0')}>
    <div class="sec">Bilateral Tissue Analysis</div>
    ...
    <table ${s('width:100%;border-collapse:collapse')}>
      <thead>
        <tr>
          <th ${thL}></th>
          <th ${thCat('lean')} colspan="3">Lean Mass (kg)</th>
          <th ${thCat('fat')} colspan="3">Fat Mass (kg)</th>
          <th ${thCat('bone')} colspan="3">Bone (kg)</th>
        </tr>
        <tr>
          <th ${thL}>Region</th>
          <th ${thStyle}>Left</th>
...
function whoViz(t, P) {
  if (t == null) {
    return `<div ${s('padding:12px;background:' + P.cardHighlight + ';border-radius:5px;font-size:9px;color:' + P.gray + ';text-align:center')}>Reference range not available for this age</div>`
  }
...
function scoreBlock(lbl, val, meaning, P) {
  if (val == null) {
    ...
    <div ${s('display:flex;flex-direction:column')}>
      ...
      <div ${s(`font-size:38px;font-weight:800;color:${P.gray};line-height:1;margin-top:4px`)}>—</div>
    </div>`
  }
...
function bodyFatRefCard(fatPct, gender, P) {
  ...
  // ACE Categories aligned to report-components.js thresholds
  // Males: [athletic(6-13), fitness(13-17), normal(17-24), overweight(24-29), obese(29+)]
  // Females: [athletic(10-20), fitness(20-24), normal(24-31), overweight(31-36), obese(36+)]
  // Colors: blue (athletic) → green (fitness) → yellow (normal) → orange (overweight) → red (obese)
  const zones = male
    ? [{ lbl: 'Athletic', range: '6–13%',   w: 12, c: '#1F77B4', lo: 6,  hi: 12.9 },   // blue
       { lbl: 'Fitness',  range: '13–17%',  w: 4.7, c: '#2CA02C', lo: 13, hi: 17 },   // green
       { lbl: 'Normal',   range: '17–24%',  w: 10.5, c: '#D4A82D', lo: 17, hi: 24 },   // dark yellow
       { lbl: 'Overweight', range: '24–29%', w: 7.5, c: C.orange, lo: 24, hi: 29 },   // orange
       { lbl: 'Obese',    range: '29%+',    w: 15.3, c: C.red,     lo: 29, hi: 50 }]  // red
    : [{ lbl: 'Athletic', range: '10–20%',  w: 16, c: '#1F77B4', lo: 10,  hi: 19.9 },   // blue
       { lbl: 'Fitness',  range: '20–24%', w: 5.1, c: '#2CA02C', lo: 20, hi: 24 },   // green
       { lbl: 'Normal',   range: '24–31%', w: 11.4, c: '#D4A82D', lo: 24, hi: 31 },   // dark yellow
       { lbl: 'Overweight', range: '31–36%', w: 8.2, c: C.orange, lo: 31, hi: 36 },   // orange
       { lbl: 'Obese',    range: '36%+',   w: 9.3, c: C.red,      lo: 36, hi: 50 }]  // red

  const firstZoneLo = zones[0].lo
  const lastZoneHi = zones[zones.length - 1].hi
  const scaleRange = lastZoneHi - firstZoneLo + 1
  const markerPct = Math.min(Math.max((fatPct - firstZoneLo) / scaleRange * 100, 0.5), 99.5)
  const zone = zones.find(z => fatPct >= z.lo && fatPct <= z.hi)
  const markerColor = zone?.c ?? P.gray

  // Build scale segments with gaps only between zones (no leading gap)
  const segments = []
  let pos = firstZoneLo

  for (const z of zones) {
    const gapWidth = z.lo - pos
    if (gapWidth > 0) {
      segments.push({ type: 'gap', w: (gapWidth / (lastZoneHi - firstZoneLo + 1)) * 100 })
    }
    const zoneWidth = (z.hi - z.lo + 1) / (lastZoneHi - firstZoneLo + 1) * 100
    segments.push({ type: 'zone', z, w: zoneWidth })
    pos = z.hi + 1
  }
  ...
  <div class="card" ${s('flex-shrink:0')}>
    ...
    <div ${s('position:relative;margin-bottom:12px')}>
      <div ${s('display:flex;height:14px;border-radius:4px;overflow:hidden;background:#e0e0e0')}>
        ${segments.map(seg =>
          seg.type === 'gap'
            ? `<div ${s(`width:${seg.w}%;background:#f0f0f0`)}></div>`
            : `<div ${s(`width:${seg.w}%;background:${seg.z.c};opacity:.5`)}></div>`
        ).join('')}
      </div>
      <div ${s(`position:absolute;left:${markerPct}%;top:-6px;bottom:-6px;width:4px;background:${P.text};border-radius:3px;transform:translateX(-50%);box-shadow:0 1px 4px rgba(0,0,0,.6);z-index:2`)}></div>
      <div ${s('position:absolute;left:0;right:0;top:20px;font-size:8px;font-weight:600;display:flex')}>
        ${zones.map((z, i) => {
          const zoneStart = (z.lo - firstZoneLo) / scaleRange * 100
          const zoneWidth = (z.hi - z.lo + 1) / scaleRange * 100
          return `<div ${s(`position:absolute;left:${zoneStart}%;width:${zoneWidth}%;text-align:center;color:${z.c}`)}>${z.lbl}</div>`
        }).join('')}
      </div>
    </div>
    <div ${s('position:relative;height:12px;margin-top:20px')}>
      ${(() => {
        const ticks = []
        // Start and end ticks
        ticks.push(`<div ${s(`position:absolute;left:0;font-size:8px;color:${P.gray};font-weight:600`)}>${firstZoneLo}%</div>`)
        // Intermediate ticks every 10%
        for (let pct = Math.ceil(firstZoneLo / 10) * 10; pct < lastZoneHi; pct += 10) {
          const pos = ((pct - firstZoneLo) / scaleRange) * 100
          ticks.push(`<div ${s(`position:absolute;left:${pos}%;transform:translateX(-50%);font-size:8px;color:${P.gray};font-weight:600`)}>${pct}%</div>`)
        }
        ticks.push(`<div ${s(`position:absolute;right:0;font-size:8px;color:${P.gray};font-weight:600`)}>${lastZoneHi}%</div>`)
        return ticks.join('')
      })()}
    </div>
    <div ${s(`font-size:10px;margin-top:8px;border-top:1px solid ${P.border};padding-top:6px;color:${P.gray}`)}>
      ...
      <span ${s(`color:${zone?.c ?? P.gray};font-weight:600;margin-left:8px`)}>${zone?.lbl ?? 'Unknown'} range</span>
    </div>
  </div>`
}
      ...
      <div class="row" ${s('align-items:stretch;gap:24px;margin-bottom:12px')}>
        <div ${s('display:flex;flex-direction:column')}>
          ...
          <div ${s('display:flex;align-items:baseline;gap:4px;margin-top:4px')}>
            <span ${s(`font-size:46px;font-weight:800;color:${boneColor(bone.classification)};line-height:1`)}>${bone.total_bmd != null ? bone.total_bmd.toFixed(3) : '—'}</span>
        ...
        ${(bone.total_t != null || bone.total_z != null) ? densitometryChart(pt.age, bone.total_bmd, 'totalbody', pt.gender, P) : ''}
      </div>
...
function page5(data, P, lh = false, pageNum = 5, totalPages = 5) {
  ...
  const male = pt.gender?.toLowerCase().startsWith('m')
  const items = summaryItems(comp, calc, bone, pt.gender, true)  // isComposition = true for totalbody standard
  ...
  const fatTargetsBlock = (() => {
    ...
    // If ALMI is low and all targets are infeasible, show recomposition guidance instead
    if (targets.almiLow) {
      return `
      <div class="card" ${s('flex-shrink:0')}>
        <div class="sec">Weight Management — Body Recomposition</div>
        <div ${s(`font-size:9px;color:${P.gray};margin-bottom:8px`)}>
          Current: <strong ${s(`color:${P.text}`)}>${targets.fatPct}% body fat · ${targets.currentWeight.toFixed(1)} kg total</strong> · Appendicular lean mass: ${targets.leanKg.toFixed(1)} kg (below normal range)
        </div>
        <div ${s(`font-size:9px;color:${P.text};padding:8px;background:${P.statusWarn};border-left:3px solid ${C.amber};margin-bottom:10px;line-height:1.6`)}}><strong>Your current lean mass doesn't support healthy weight loss targets.</strong> Numeric weight-loss targets aren't shown because they would place you in an underweight BMI range. Instead, focus on body recomposition: building lean mass while managing fat.</div>
        <div ${s(`font-size:9px;color:${P.text};padding:10px;background:${P.cardHighlight};border-radius:4px;line-height:1.6`)}}>
          <strong>Recommended priority: Resistance training</strong>
          <ul ${s('margin:6px 0 0 0;padding-left:20px')}>
            <li>2–3 sessions per week of resistance training (weights, bodyweight, or bands)</li>
            <li>Protein intake of ≥1.6 g/kg body weight per day supports muscle gain</li>
            <li>Aim for modest caloric deficit (200–300 kcal/day) or maintenance calories while training</li>
            <li>Re-scan in 3–6 months — once appendicular lean mass approaches the normal range (${(Math.sqrt(comp.lean_g / 1000 / 7.26) * 7.26).toFixed(1)} kg), meaningful fat-loss targets can be calculated</li>
          </ul>
        </div>
      </div>`
    }

    const rows = targets.targets.map(({ pct, weightRange, fatLossRange }) => {
      return `<tr>
        <td ${s(`padding:6px 10px;font-size:11px;color:${P.gray}`)}>${pct}% body fat</td>
        <td ${s(`padding:6px 10px;font-size:11px;color:${C.amber};font-family:monospace;text-align:right`)}>${fatLossRange} kg</td>
        <td ${s(`padding:6px 10px;font-size:11px;color:${P.text};font-family:monospace;text-align:right`)}>${weightRange} kg</td>
      </tr>`
    }).join('')
    ...
    return `
    <div class="card" ${s('flex-shrink:0')}>
      <div class="sec">Body Composition Goals — Fat Loss &amp; Lean Mass Targets</div>
      <div ${s(`font-size:8px;color:${P.gray};margin-bottom:8px`)}><strong>Current:</strong> Weight <strong>${targets.currentWeight.toFixed(1)} kg</strong> · Fat <strong>${targets.currentFat.toFixed(1)} kg</strong> · Lean <strong>${targets.currentLean.toFixed(1)} kg</strong> · Body fat <strong>${targets.fatPct}%</strong> · ALMI <strong>${targets.currentAlmi}</strong></div>
      <div ${s(`font-size:9px;color:${targets.almiStatus === 'low' ? C.orange : P.text};padding:6px 8px;background:${targets.almiStatus === 'low' ? P.statusWarn : P.statusInfo};border-left:3px solid ${targets.almiStatus === 'low' ? C.amber : C.tealLt};margin-bottom:8px;line-height:1.5`)}>${targets.almiGuidance} ${targets.preservationNote}</div>
      <table ${s('width:100%;border-collapse:collapse;margin:6px 0 10px 0')}>
        <thead><tr>
          ...
          <th ${s(`padding:4px 10px;font-size:9px;font-weight:700;color:${P.gray};text-align:right;border-bottom:1px solid ${P.border}`)}>Fat to Lose</th>
          <th ${s(`padding:4px 10px;font-size:9px;font-weight:700;color:${P.gray};text-align:right;border-bottom:1px solid ${P.border}`)}>Weight Range</th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
      <div ${s(`font-size:8px;color:${P.gray};margin-bottom:6px;line-height:1.4`)}><em>Weight ranges assume maintenance or modest gains in lean mass through exercise. Repeat DEXA after 5–10% weight loss, 3–6 months of exercise, or to confirm fat loss vs muscle loss.</em></div>
      ${sarcoWarning}
    </div>`
  })()
...
<div class="page">
  ...
  <div class="card">
    ...
    <div class="row" ${s('gap:14px;align-items:flex-start')}>
      ...
      <div ${s(`width:1px;background:${P.border};align-self:stretch`)}></div>
      <div ${s('flex:1')}>
        <div ${s(`font-size:8px;font-weight:700;color:${P.gray};text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px`)}>Daily Energy Needs (kcal/day)</div>
        <div ${s('display:grid;grid-template-columns:1fr 1fr;gap:2px 10px')}>
          ${[['Sedentary',1.2],['Light',1.375],['Moderate',1.55],['Active',1.725]].map(([lbl,f]) => {
            const kcal = Math.round(calc.rmr_kcal * f)
            return `<div ${s('display:flex;justify-content:space-between;align-items:baseline;padding:2px 0;border-bottom:1px solid ' + P.border)}>
              <span ${s(`font-size:9px;color:${P.gray}`)}>${lbl}</span>
              <span ${s(`font-size:9.5px;color:${C.tealLt};font-family:monospace`)}>${kcal.toLocaleString()}</span>
            </div>`
          }).join('')}
        </div>
        <div ${s(`font-size:8px;color:${P.gray};margin-top:4px;line-height:1.4`)}>Adjust ±500 kcal/day for ~0.5 kg/week weight loss or gain.</div>
      </div>
    </div>
  </div>
