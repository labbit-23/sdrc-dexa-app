...
function bodyFatRefCard(fatPct, gender, P) {
  ...
  <div class="card" ${s('flex-shrink:0')}>
    ...
    <div ${s('position:relative;height:16px;margin-top:20px')}>
      ${(() => {
        const ticks = []
        // Show category label with range at each zone boundary
        zones.forEach((z, i) => {
          const pos = ((z.lo - firstZoneLo) / scaleRange) * 100
          const rangeLabel = z.range.replace('–', '-')
          ticks.push(`<div ${s(`position:absolute;left:${pos}%;transform:translateX(-50%);font-size:8px;color:${z.c};font-weight:600;white-space:nowrap;line-height:1.2`)}>${z.lbl}<br/>(${rangeLabel})</div>`)
        })
        return ticks.join('')
      })()}
    </div>
