    ...
    <div class="col" ${s('flex:1;gap:12px;min-height:0')}>
      ...
      <div class="card" ${s('flex-shrink:0')}>
        ...
        <div class="row" ${s('gap:16px;align-items:flex-start')}>
          <div ${s('flex:1')}>
            <div ${s('display:flex;align-items:baseline;gap:6px;margin-bottom:6px')}>
              <span ${s(`font-size:28px;font-weight:800;color:${C.tealLt};line-height:1`)}>${calc.rmr_kcal.toLocaleString()}</span>
              <span ${s(`font-size:11px;color:${P.gray}`)}>kcal / day</span>
            </div>
            <div ${s(`font-size:9.5px;color:${P.gray};line-height:1.7`)}>
              Estimated calories burned <strong ${s(`color:${P.text}`)}>at complete rest</strong> — breathing, circulation, organ function.
              Calculated from lean mass using the Katch-McArdle formula, which is more accurate than weight-based equations.
            </div>
          </div>
          <div ${s(`width:1px;background:${P.border};align-self:stretch`)}></div>
          <div ${s('flex:1')}>
            <div ${s(`font-size:9px;font-weight:700;color:${P.gray};text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px`)}>Estimated Daily Energy Needs</div>
            ${[
              ['Sedentary', 1.2,  'desk job, little exercise'],
              ['Light',     1.375,'light exercise 1–3×/week'],
              ['Moderate',  1.55, 'exercise 3–5×/week'],
              ['Active',    1.725,'hard exercise 6–7×/week'],
            ].map(([lbl, f, note]) => {
              const kcal = Math.round(calc.rmr_kcal * f)
              return `<div ${s('display:flex;justify-content:space-between;align-items:baseline;padding:3px 0;border-bottom:1px solid ' + P.border)}>
                <div>
                  <span ${s(`font-size:9.5px;font-weight:600;color:${P.text}`)}>${lbl}</span>
                  <span ${s(`font-size:8.5px;color:${P.gray};margin-left:5px`)}>${note}</span>
                </div>
                <span ${s(`font-size:10px;font-weight:700;color:${C.tealLt};font-family:monospace`)}>${kcal.toLocaleString()} kcal</span>
              </div>`
            }).join('')}
            <div ${s(`font-size:8px;color:${P.gray};margin-top:5px`)}>Higher lean mass → higher RMR → easier weight management.</div>
          </div>
        </div>
      </div>
      <div class="card" ${s('flex-shrink:0')}>
