...
function muscleContextCard(P, leanPreserved = false) {
  const heading = leanPreserved ? 'Muscle Mass: Well Preserved' : 'Muscle Mass — What to Know'
  const intro = leanPreserved
    ? `Your appendicular lean mass (ALMI) is within or above the normal reference range — this is a significant positive finding. Maintaining muscle while managing body fat is the most effective strategy for long-term metabolic health.`
    : `Skeletal muscle mass peaks around age 25–30 and naturally declines <strong ${s(`color:${P.text}`)}>3–8% per decade</strong> if not actively maintained. Low appendicular lean mass is linked to reduced strength, insulin resistance, and higher metabolic risk.`
  return `
  <div class="card" ${s('flex-shrink:0')}>
    <div class="sec">${heading}</div>
    <div ${s(`font-size:10.5px;color:${P.gray};line-height:1.85`)}>
      ${intro}<br>
      <span ${s(`color:${C.tealLt};font-weight:600`)}>▸ Resistance training</span> 2–3× per week is the most effective way to build and maintain lean mass at any age.<br>
      <span ${s(`color:${C.tealLt};font-weight:600`)}>▸ Protein intake</span> of ≥ 1.6 g/kg body weight/day supports muscle maintenance during weight loss.
    </div>
  </div>`
}
...
function page2(data, P, lh = false, pageNum = 2, totalPages = 5) {
  ...
  <div class="card" ${s('flex-shrink:0')}>
    ...
    <div ${s('display:flex;align-items:center;gap:12px')}>
      ...
      <div ${s('flex:1')}>
        <div ${s(`font-size:10px;color:${P.text};font-weight:600`)}>Body fat % is greater than that of ${c}% of ${gender === 'Male' ? 'men' : 'women'} of the same age</div>
...
<div class="page">
  ...
  <div ${s('flex:1;display:flex;flex-direction:column;gap:14px;min-height:0')}>
    ...
    ${muscleContextCard(P, calc.lean_preserved)}
...
function page5(data, P, lh = false, pageNum = 5, totalPages = 5) {
  ...
  const fatTargetsBlock = calc.fat_loss_targets && comp.fat_pct > bfThreshold ? (() => {
    const currentWeight = parseFloat((comp.fat_g / 1000 + comp.lean_g / 1000 + comp.bmc_g / 1000).toFixed(1))
    const rows = calc.fat_loss_targets.map(({ pct, target_kg, loss_kg }) => {
      const reachable = loss_kg > 0
      return `<tr>
        <td ${s(`padding:5px 10px;font-size:11px;color:${P.gray}`)}>${pct}% body fat</td>
        <td ${s(`padding:5px 10px;font-size:11px;font-weight:700;color:${P.text};font-family:monospace;text-align:right`)}>${target_kg} kg</td>
        <td ${s(`padding:5px 10px;font-size:11px;font-weight:700;color:${reachable ? C.amber : C.greenLt};font-family:monospace;text-align:right`)}>${reachable ? `−${loss_kg} kg` : '✓ achieved'}</td>
      </tr>`
    }).join('')
    return `
    <div class="card" ${s('flex-shrink:0')}>
      <div class="sec">Weight Goal — Fat Loss Targets</div>
      <div ${s(`font-size:9px;color:${P.gray};margin-bottom:6px`)}>Current: <strong ${s(`color:${P.text}`)}>${comp.fat_pct}% body fat · ${currentWeight} kg total</strong> · Based on preserving current lean mass of ${(comp.lean_g/1000).toFixed(1)} kg</div>
      <table ${s('width:100%;border-collapse:collapse')}>
        <thead><tr>
