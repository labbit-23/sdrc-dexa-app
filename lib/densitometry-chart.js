...
export function densitometryChart(age, bmd, site, gender, P) {
  ...
  const fRef = _bmNorm && _bmNorm.forearm
  const tbRef = _bmNorm && _bmNorm.totalbody
  if (!nRef || !sRef || !tbRef || !fRef) return ''

  // Hip always uses female reference (ISCD standard); spine, forearm & total body are gender-specific
  const { peak, sd, curve } = isTotalBody
    ? (male ? tbRef.male : tbRef.female)
    : isForearm
    ? (male ? fRef.male : fRef.female)
    : isSpine
    ? (male ? sRef.male : sRef.female)
    : nRef.female

  // Unified chart dimensions (forearm taller to match long image)
  const W = isTotalBody ? 220 : 230
  const H = isTotalBody ? 112 : isForearm ? 140 : 140
  ...
  // Adjust age range based on patient age — show appropriate context
  const AGE_MIN = age < 25 ? 5 : age > 70 ? 40 : 20
  const AGE_MAX = age < 25 ? 20 : 90
  let BMD_MIN = isTotalBody ? 0.70 : isSpine ? 0.54 : isForearm ? 0.35 : 0.40
  let BMD_MAX = isTotalBody ? 1.40 : isSpine ? 1.50 : isForearm ? 1.20 : 1.20

  // Extend range if actual BMD is outside bounds
  if (bmd < BMD_MIN) BMD_MIN = bmd - 0.05
  if (bmd > BMD_MAX) BMD_MAX = bmd + 0.05
  ...
  // For young patients (age < 25): use Z-score thresholds (age-matched); for adults: use T-score
  const isYoung = age < 25

  // Find age-specific reference values by interpolating the curve
  let refBmdAtAge = peak
  if (isYoung && curve.length > 0) {
    for (let i = 0; i < curve.length - 1; i++) {
      const [a1, b1] = curve[i]
      const [a2, b2] = curve[i + 1]
      if (age >= a1 && age <= a2) {
        refBmdAtAge = b1 + (b2 - b1) * (age - a1) / (a2 - a1)
        break
      }
    }
  }

  // Zone boundaries: use age-matched reference for young patients, population peak for adults
  const zoneRef = isYoung ? refBmdAtAge : peak
  const zoneSD = sd
  const zOst = zoneRef - zoneSD          // Z = −1 or T = −1
  const zOstP = zoneRef - 2 * zoneSD    // Z = −2 (young) or T = −2.5 (adult)
  const zOstPA = zoneRef - 2.5 * zoneSD // T = −2.5 (for adults)
  ...
  const c1  = curvePts(1)                              // Z = −1 (young) or T = −1 (adult)
  const c25 = curvePts(isYoung ? 2 : 2.5)              // Z = −2 (young) or T = −2.5 (adult)

  const pts = arr => arr.map(([x, y]) => `${x.toFixed(1)},${cy(y).toFixed(1)}`).join(' ')

  // Horizontal zone fills (age-matched for young patients, T-score based for adults)
  const yNorm  = cy(yp(zOst))
  const yOstP  = isYoung ? cy(yp(zOstP)) : cy(yp(zOstPA))

  // Zone label X: just inside left edge; Y: midpoints between zone boundaries
  ...
  // Patient dot — color reflects zone (age-matched for young patients, T-score for adults)
  const dotX = Math.max(ml + 5, Math.min(ml + cW - 5, xp(age)))
  const dotY = Math.max(mt + 5, Math.min(mt + cH - 5, yp(bmd)))
  const dotColor = isYoung
    ? (bmd <= zOstP ? C.red : bmd <= zOst ? C.amber : C.greenLt)
    : (bmd <= zOstPA ? C.red : bmd <= zOst ? C.amber : C.greenLt)
  const lblY = dotY > mt + 13 ? (dotY - 9).toFixed(1) : (dotY + 17).toFixed(1)

  // Dynamic age ticks based on visible age range
  const xTicks = AGE_MIN === 5 ? [5, 10, 15, 20] : [20, 30, 40, 50, 60, 70, 80, 90]
  ...
  <div ${st('flex:1;display:flex;flex-direction:column;min-width:0')}>
    ...
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${totalW} ${H}" width="100%" style="display:block;flex:1">
      ...
      <!-- Horizontal zone fills (T-score for adults only; young patients use sloping age-matched zones) -->
      ${isYoung ? '' : `
      ...
      <!-- Zone boundary lines with labels (T-score for adults, Z-score for young) -->
      ${isYoung ? `
      <text x="${(ml+2).toFixed(1)}" y="${(yp(zoneRef - 1*zoneSD)-1.5).toFixed(1)}" font-size="5" fill="#888">Z=−1</text>
      <text x="${(ml+2).toFixed(1)}" y="${(yp(zoneRef - 2*zoneSD)-1.5).toFixed(1)}" font-size="5" fill="#888">Z=−2</text>`
      : `
