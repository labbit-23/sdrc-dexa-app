...
export function computeOsteoData(raw, patientId, imageBaseUrl) {
  ...
  //   True total-body scan: fat_g/lean_g in grams; has Arms/Legs/Trunk regions
  //   Osteo estimated:      fat_g/lean_g = percentage×10; only Total/Android/Gynoid
  const compRaw = session.estimated_composition || {}
  const totalComp = compRaw.Total
  const heightM   = (pat.height_cm || 0) / 100

  // True total-body if Legs or Trunk present (labels 2/3 — only in full body scans).
  // Arms (label=1) exists in both total-body and osteo estimated, so not a reliable indicator.
  const isTrueTotalBody = !!(compRaw.Legs || compRaw.Trunk)

  const compFatPct = totalComp?.fat_pct ?? null

  // kg and index only meaningful for true total-body (values are actual grams)
  const compFatKg  = isTrueTotalBody && totalComp ? +(totalComp.fat_g  / 1000).toFixed(1) : null
  const compLeanKg = isTrueTotalBody && totalComp ? +(totalComp.lean_g / 1000).toFixed(1) : null
  const compFmi    = heightM > 0 && compFatKg  != null ? +(compFatKg  / heightM ** 2).toFixed(1) : null
  const compLmi    = heightM > 0 && compLeanKg != null ? +(compLeanKg / heightM ** 2).toFixed(1) : null
