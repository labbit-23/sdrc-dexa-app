...
export function computeOsteoData(raw, patientId, imageBaseUrl) {
  // Support both formats: {patient, session} (legacy) and {mdb_snapshot, xps_bone} (trends)
  let pat, session, spine, left_femur, right_femur

  if (raw.mdb_snapshot) {
    // Format: {mdb_snapshot, xps_bone} — used by trends
    const snap = raw.mdb_snapshot
    const patRow = Object.values(snap.patients || {})[0]
    const examRow = snap.exams?.[0]

    pat = {
      name: patRow?.name_last || '',
      title: patRow?.name_first || '',
      dob: patRow?.dob || '',
      gender: patRow?.gender || 'Female',
      height_cm: patRow?.height_cm || 0,
      weight_kg: patRow?.weight_kg || 0,
      bmi: 0,
      physician: '',
    }
    session = {
      scan_date: examRow?._acq_dt || new Date().toISOString(),
      spine: snap.spine || {},
      left_femur: snap.left_femur || {},
      right_femur: snap.right_femur || {},
      estimated_composition: snap.estimated_composition || {},
      scanner_serial: snap.scanner_serial || '',
      software: snap.software || '',
    }
  } else {
    // Format: {patient, session} — legacy direct format
    pat = raw.patient
    session = raw.session
  }
  ...
  spine       = parseSpine(session.spine)
  left_femur  = parseFemur(session.left_femur)
  right_femur = parseFemur(session.right_femur)
