...
import { forearmSummary } from './report-components.js'

function classify(t) {
  if (t == null) return 'unknown'
  // Compare raw T-score, not the rounded display value — rounding first
  // can only push borderline osteopenia scores into osteoporosis, never
  // the reverse (see Hip Classification badge in osteo-html-template.js
  // for the matching raw comparison).
...
function parseForearm(raw) {
  ...
  for (const [key, val] of Object.entries(raw || {})) {
    const r = parseRegion(val)
    if (!r) continue
    // Accept all forearm regions: Radius/Ulna UD/33%/Total, Both UD/33%/Total
    if (['Radius UD', 'Ulna UD', 'Radius 33%', 'Ulna 33%', 'Radius Total', 'Ulna Total', 'Both UD', 'Both 33%', 'Both Total'].includes(key)) {
      out[key] = r
    }
  }
  return out
...
export function computeOsteoData(raw, patientId, imageBaseUrl) {
  ...
  // Get forearm diagnostic values (Radius 33%)
  const forearmSummaryData = forearmSummary(left_forearm, right_forearm)
  const forearm_t = forearmSummaryData?.primaryDiagnostic?.T ?? null
  const forearm_z = forearmSummaryData?.primaryDiagnostic?.Z ?? null
  const forearm_has_left = forearmSummaryData?.hasLeft ?? false
  const forearm_has_right = forearmSummaryData?.hasRight ?? false

  // Determine which side is driving the forearm T-score
  let forearm_side = null
  if (forearmSummaryData) {
    const leftR33 = left_forearm?.['Radius 33%']
    const rightR33 = right_forearm?.['Radius 33%']
    if (leftR33 && rightR33) {
      // Both present: use the side with lower (worse) T-score
      forearm_side = leftR33.T != null && rightR33.T != null && leftR33.T <= rightR33.T ? 'left' : 'right'
    } else if (leftR33) {
      forearm_side = 'left'
    } else if (rightR33) {
      forearm_side = 'right'
  ...
  // True minimum across ISCD diagnostic sites only (spine L1-L4, hip Neck/Total,
  // forearm Radius 33%) — not a hip/spine/forearm priority fallback, since any
  // of the three can be the one that actually crosses a WHO threshold.
  const overall_t = lowestT(spine_t, lowest_hip_t, forearm_t)
  const overall_class = classify(overall_t)
  const overall_site =
    overall_t == null ? null :
    overall_t === spine_t ? 'spine' :
    overall_t === lowest_hip_t ? 'hip' :
    'forearm'
  const overall_z =
    overall_site === 'spine' ? spine_z :
    overall_site === 'hip' ? lowest_hip_z :
    overall_site === 'forearm' ? forearm_z :
    null
  ...
  return {
    ...
    left_forearm,
    right_forearm,
    forearm_summary: forearmSummary(left_forearm, right_forearm),
    ...
    summary: {
      ...
      forearm_t,
      forearm_z,
      forearm_class: classify(forearm_t),
      forearm_side,
      forearm_has_left,
      forearm_has_right,
      overall_class,
    ...
    images: {
      ...
      left_forearm_url:  imageBaseUrl ? `${imageBaseUrl}/img_left_forearm.png`  : '',
      right_forearm_url: imageBaseUrl ? `${imageBaseUrl}/img_right_forearm.png` : '',
    },
  }
}
