...
export function computeOsteoData(raw, patientId, imageBaseUrl) {
  ...
  // Two label sets exist in the MDB:
  //   True total-body scan: fat_g/lean_g in grams; has Arms/Legs/Trunk regions (labels 2/3)
  //   Osteo estimated:      fat_g/lean_g = percentage×10; only Total/Android/Gynoid
  //
  // For combined patients (osteo + total body both uploaded), the MDB snapshot captures
  // composition across ALL img_handles, so total-body regions bleed into the osteo raw_json.
  // When true total-body data is detected here, suppress the section entirely — those values
  // belong in the Total Body report, not the osteo report.
  const compRaw = session.estimated_composition || {}

  // True total-body contamination detected if Legs or Trunk present (labels 2/3 — full body only).
  // Arms (label=1) exists in both, so not a reliable indicator.
  const hasTotalBodyData = !!(compRaw.Legs || compRaw.Trunk)

  // Only use osteo estimated composition — discard if total-body data is present
  const totalComp = !hasTotalBodyData ? (compRaw.Total ?? null) : null

  const estimated_composition = totalComp ? {
    fat_pct:         totalComp.fat_pct ?? null,
    fat_kg:          null,    // osteo estimated does not give true mass in grams
    lean_kg:         null,
    fmi:             null,
    lmi:             null,
    android_fat_pct: compRaw.Android?.fat_pct ?? null,
    gynoid_fat_pct:  compRaw.Gynoid?.fat_pct  ?? null,
    is_estimated:    true,    // always estimated when sourced from osteo scan
  } : null
