...
import { summaryItems, fatLossTargets, scanDelta, muscleContext, boneGuide, almiBadge, boneClassBadge, centileText, aceCategory } from './report-components.js'
...
function page1(data, lh, pageNum, total) {
  ...
  // Use ACE category from report-components instead of hardcoded thresholds
  const aceData = aceCategory(comp.fat_pct, pt.gender)
  const aceTicks = male
    ? ['&lt; 13', '13–17', '17–24', '24–29', '&gt; 29']
    : ['&lt; 20', '20–24', '24–31', '31–36', '&gt; 36']
  const aceMarkerPct = (comp.fat_pct / aceMax) * 100

  const aceLabel = aceData.level === 'athletic' ? 'Athletic' : aceData.level === 'fitness' ? 'Fit range' : aceData.level === 'normal' ? 'Normal range' : aceData.level === 'overweight' ? 'Overweight' : 'Obese'
  const acePillCls = aceData.level === 'athletic' || aceData.level === 'fitness' || aceData.level === 'normal' ? 'sage' : aceData.level === 'overweight' ? 'amber' : 'accent'
    ...
    <div class="with-scan">
      ...
      <div class="col">
        ...
        <div>
          ...
          <div style="margin-top:4px;font-size:10px;font-weight:600;color:var(--ink)">Category: ${aceLabel}</div>
        </div>
      </div>
    </div>
  </div>
...
function page5(data, lh, pageNum, total) {
  ...
  // Get standardized summary items from report-components.js
  const summaryData = summaryItems(comp, calc, bone, pt.gender, true)

  // Map summary items to editorial findings format
  const findings = summaryData.map(item => ({
    v: item.status === 'good' ? '' : item.status === 'warn' ? 'caution' : item.status === 'alert' ? 'attention' : 'info',
    pill: item.status === 'good' ? 'sage' : item.status === 'warn' ? 'amber' : 'accent',
    pt: item.title,
    h: item.title,
    b: item.body
  }))
...
<section class="page">
  ...
  <div class="pb">
    ...
    ${(() => {
      ...
      if (!targets || !targets.targets || targets.targets.length === 0) return ''
      const rows = targets.targets.map(({ pct, weightRange, fatLossRange }) => {
        return `<tr>
          ...
          <td style="padding:4px 8px;font-size:10px;color:var(--muted);text-align:right">${fatLossRange} kg</td>
          <td style="padding:4px 8px;font-size:10px;font-weight:600;color:var(--ink);text-align:right">${weightRange} kg</td>
        </tr>`
      }).join('')
      return `
        ...
        <div>
          ...
          <table style="width:100%;border-collapse:collapse;font-size:9px">
            <thead><tr>
              ...
              <th style="padding:3px 8px;text-align:right;border-bottom:1px solid var(--hairline);color:var(--muted);font-weight:600">Fat Loss</th>
              <th style="padding:3px 8px;text-align:right;border-bottom:1px solid var(--hairline);color:var(--muted);font-weight:600">Weight Range</th>
            </tr></thead>
            <tbody>${rows}</tbody>
          </table>
