...
function kg(g)  { return (g / 1000).toFixed(1) }
function kg2(g) { return (g / 1000).toFixed(2) }
...
<section class="page">
      ...
      <div class="col">
        ...
        <div style="display:grid;grid-template-columns:1fr auto;gap:8mm;align-items:end">
          ...
          <aside style="text-align:right">
            <div class="kicker">Age-matched centile</div>
            <div style="font-family:'Instrument Serif',serif;font-size:48px;line-height:1;letter-spacing:-.02em;margin:2mm 0;color:var(--sage)">${comp.centile}<span style="font-size:18px;color:var(--muted);font-style:italic">th</span></div>
            <div>${pill(acePillCls, aceLabel)}</div>
  ...
  ${pageFooter('Image not for diagnosis', pageNum, total)}
</section>`
}
  ...
  <div class="pb">
    <div class="with-scan">
      <div class="scan tall">
        ...
        <div class="scan-caption">Dense fat · Low fat</div>
        <div class="scan-disc">Image not for diagnosis</div>
      </div>
      <div class="col">
        <!-- A/G ratio -->
        ...
        <div>
          ...
          <div style="display:flex;align-items:center;gap:8mm">
            ...
            <div class="copy">
              <p>Body fat higher than ${comp.centile}% of same-age, same-sex peers.</p>
  ...
  <div class="pb">
    <div class="with-scan">
      <div class="scan tall">
        ...
        <img class="scan-img" src="${esc(data.images.bone_url)}" alt="Bone DEXA scan" />
      ...
      <div class="col">
        ...
        <div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:6mm;align-items:end">
            <div>
...
<section class="page">
  ...
  <div class="pb">
    ...
    <div class="row-2a">
      ...
      <div>
        <div class="ptitle"><small>Total daily energy needs</small>RMR × activity level (kcal / day)</div>
        <div class="activity">
          ${tdee.map(t => `<div><div class="lvl">${esc(t.lvl)}</div><div class="kcal">${t.kcal.toLocaleString()}</div></div>`).join('')}
        </div>
        <div class="copy" style="margin-top:2mm">
          <p style="font-size:9.5px;color:var(--muted)">Multiply RMR by activity factor to estimate daily calorie needs for weight maintenance. Adjust ±500 kcal/day for ~500 g/week loss or gain.</p>
        </div>
      </div>
    </div>
    <hr class="hair" />
    <div style="display:grid;grid-template-columns:auto 1fr;gap:6mm;align-items:start">
      <div style="border-right:1px solid var(--hairline);padding-right:6mm">
        <div style="display:inline-flex;flex-direction:column;align-items:center;gap:2mm;padding:4mm 5mm;border:1px solid var(--accent-soft);background:#F4DDD3">
          <div style="font-family:'Instrument Serif',serif;font-style:italic;font-size:20px;line-height:1;color:var(--accent)">Gold</div>
          <div style="font-family:'Instrument Serif',serif;font-style:italic;font-size:20px;line-height:1;color:var(--accent)">standard</div>
          <div style="font-size:8.5px;color:var(--muted);letter-spacing:.12em;text-transform:uppercase;margin-top:1mm">WHO endorsed</div>
        </div>
      </div>
      <div>
        <div class="ptitle"><small>Why DEXA?</small>Method &amp; caveats</div>
        <div class="copy">
          <p>DEXA uses two X-ray energies to precisely separate fat, lean and bone tissue — the only method that directly measures all three. Radiation dose &lt; 1 μSv, less than a single day of background exposure.</p>
          <p style="font-size:9.5px;color:var(--muted)">Reference population: ${esc(pt.ethnicity || 'White')} (GE Lunar normative database). South Asian-specific T &amp; Z score references are not available — results may underestimate bone loss risk in South Asian individuals. VAT estimation not available on this scanner (GE Lunar DPX-NT). Trend comparison after repeat scans on the same scanner. <strong>For clinical use only — interpret with a qualified clinician.</strong></p>
        </div>
      </div>
    </div>
  </div>
