...
// 2026-08-11: 15s request timeout on every Supabase fetch so a slow/stuck
// DB response fails fast instead of hanging the request handler forever.
const withTimeout = (input, init = {}) => fetch(input, { ...init, signal: AbortSignal.timeout(15000) })

/** Browser / RSC client (anon key — read-only) */
export const supabase = createClient(url, anon, { global: { fetch: withTimeout } })
...
export function getServiceClient() {
  return createClient(url, service, {
    global: {
      ...
      // pages always show fresh data (not stale cached rows), plus the
      // same request timeout as the anon client above.
      fetch: (input, init) => fetch(input, { ...init, cache: 'no-store', signal: AbortSignal.timeout(15000) }),
    },
  })
}
